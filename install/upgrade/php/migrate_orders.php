...
function migrate_orders()
{
    ...
    $order_res = Db::getInstance()->query('SELECT *
			FROM `' . _DB_PREFIX_ . 'orders`');
    // this was done like that previously
    $wrapping_tax_rate = 1 + (double) Db::getInstance()->getValue('SELECT value
    ...
    while ($order = Db::getInstance()->nextRow($order_res)) {
        ...
        foreach ($order_details_list as $order_details) {
            ...
            $tax_rate = 1 + (double) $products['tax_rate'] / 100;
            $reduction_amount_tax_incl = (double) $products['reduction_amount'];
            ...
            $order_details['reduction_amount_tax_excl'] = (double) mo_ps_round($reduction_amount_tax_incl / $tax_rate);
            $order_details['total_price_tax_incl'] = (double) $products['total_wt'];
            $order_details['total_price_tax_excl'] = (double) $products['total_price'];
            $order_details['unit_price_tax_incl'] = (double) $products['product_price_wt'];
            $order_details['unit_price_tax_excl'] = (double) $products['product_price'];
            foreach (array_keys($order_details) as $k) {
                if (!in_array($k, $col_order_detail)) {
                    ...
                    if (in_array($order_details[$k], array('product_price', 'reduction_percent', 'reduction_amount', 'group_reduction', 'product_quantity_discount', 'tax_rate', 'ecotax', 'ecotax_tax_rate'))) {
                        $order_details[$k] = (double) $order_details[$k];
                    } else {
        ...
        $carrier_tax_rate = 1 + (double) $order['carrier_tax_rate'] / 100;
        $total_discount_tax_excl = $order['total_discounts'] / $average_tax_used;
        $order['total_discounts_tax_incl'] = (double) $order['total_discounts'];
        $order['total_discounts_tax_excl'] = (double) $total_discount_tax_excl;
        $order['total_shipping_tax_incl'] = (double) $order['total_shipping'];
        $order['total_shipping_tax_excl'] = (double) ($order['total_shipping'] / $carrier_tax_rate);
        $shipping_taxes = $order['total_shipping_tax_incl'] - $order['total_shipping_tax_excl'];
        $order['total_wrapping_tax_incl'] = (double) $order['total_wrapping'];
        $order['total_wrapping_tax_excl'] = (double) $order['total_wrapping'] / $wrapping_tax_rate;
        ...
        $order['total_paid_tax_incl'] = (double) $order['total_paid'];
        $order['total_paid_tax_excl'] = (double) $order['total_paid'] - $shipping_taxes - $wrapping_taxes - $product_taxes;
