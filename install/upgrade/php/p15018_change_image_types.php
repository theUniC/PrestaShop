...
function p15018_change_image_types()
{
    ...
    $replace_types = array('products' => array('small' => 'small_default', 'medium' => 'medium_default', 'large' => 'large_default', 'thickbox' => 'thickbox_default', 'home' => 'home_default'), 'others' => array('category' => 'category_default', 'large_scene' => 'scene_default', 'thumb_scene' => 'm_scene_default'));
        ...
        foreach ($result as $row) {
            if (file_exists(_PS_ROOT_DIR_ . DIRECTORY_SEPARATOR . 'img' . DIRECTORY_SEPARATOR . 'p' . DIRECTORY_SEPARATOR . $row['id_product'] . '-' . $row['id_image'] . '.jpg')) {
                foreach ($replace_types['products'] as $old_type => $new_type) {
                    p15018_copy_or_rename(_PS_ROOT_DIR_ . DIRECTORY_SEPARATOR . 'img' . DIRECTORY_SEPARATOR . 'p' . DIRECTORY_SEPARATOR . $row['id_product'] . '-' . $row['id_image'] . '-' . $old_type . '.jpg', _PS_ROOT_DIR_ . DIRECTORY_SEPARATOR . 'img' . DIRECTORY_SEPARATOR . 'p' . DIRECTORY_SEPARATOR . $row['id_product'] . '-' . $row['id_image'] . '-' . $new_type . '.jpg', $option);
                }
            }
            ...
            if (file_exists(_PS_ROOT_DIR_ . DIRECTORY_SEPARATOR . 'img' . DIRECTORY_SEPARATOR . 'p' . DIRECTORY_SEPARATOR . $folder . $row['id_image'] . '.jpg')) {
                foreach ($replace_types['products'] as $old_type => $new_type) {
                    p15018_copy_or_rename(_PS_ROOT_DIR_ . DIRECTORY_SEPARATOR . 'img' . DIRECTORY_SEPARATOR . 'p' . DIRECTORY_SEPARATOR . $folder . $row['id_image'] . '-' . $old_type . '.jpg', _PS_ROOT_DIR_ . DIRECTORY_SEPARATOR . 'img' . DIRECTORY_SEPARATOR . 'p' . DIRECTORY_SEPARATOR . $folder . $row['id_image'] . '-' . $new_type . '.jpg', $option);
                }
            }
        }
                ...
                foreach ($replace_types as $type => $type_array) {
                    foreach ($type_array as $old_type => $new_type) {
                        if (preg_match('/^([0-9]+|[a-z]{2}-default)\\-' . $old_type . '\\.jpg$/i', $file, $matches)) {
                            p15018_copy_or_rename(_PS_ROOT_DIR_ . DIRECTORY_SEPARATOR . 'img' . DIRECTORY_SEPARATOR . $directory . DIRECTORY_SEPARATOR . $matches[1] . '-' . $old_type . '.jpg', _PS_ROOT_DIR_ . DIRECTORY_SEPARATOR . 'img' . DIRECTORY_SEPARATOR . $directory . DIRECTORY_SEPARATOR . $matches[1] . '-' . $new_type . '.jpg', $option);
                        }
                    }
                }
            }
