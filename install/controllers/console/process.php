...
{
    use \InstallModelInstall;
    use \InstallModelDatabase;
    ...
    public function process()
    {
        ...
        if ($this->datas->newsletter) {
            $params = http_build_query(array('email' => $this->datas->admin_email, 'method' => 'addMemberToNewsletter', 'language' => $this->datas->lang, 'visitorType' => 1, 'source' => 'installer'));
    ...
    {
        return $this->model_install->generateSettingsFile($this->datas->database_server, $this->datas->database_login, $this->datas->database_password, $this->datas->database_name, $this->datas->database_prefix, $this->datas->database_engine);
    }
    /**
    ...
    public function processSendEmail()
    {
        ...
        $mail = new InstallModelMail($this->datas->use_smtp, $this->datas->smtp_server, $this->datas->smtp_login, $this->datas->smtp_password, $this->datas->smtp_port, $this->datas->smtp_encryption, $this->datas->admin_email);
        ...
        $vars = array('{firstname}' => $this->datas->admin_firstname, '{lastname}' => $this->datas->admin_lastname, '{shop_name}' => $this->datas->shop_name, '{passwd}' => $this->datas->admin_password, '{email}' => $this->datas->admin_email, '{shop_url}' => Tools::getHttpHost(true) . __PS_BASE_URI__);
        $content = str_replace(array_keys($vars), array_values($vars), $content);
        $mail->send($this->l('%s - Login information', $this->datas->shop_name), $content);
        return true;
    }
    /**
