...
use Prestashop\Installer\ControllerHttp;
use Prestashop\Installer\Languages;
use Prestashop\Installer\Model\Install;
...
class InstallControllerHttpProcess extends ControllerHttp
{
    ...
    {
        $success = $this->model_install->generateSettingsFile($this->session->database_server, $this->session->database_login, $this->session->database_password, $this->session->database_name, $this->session->database_prefix, $this->session->database_engine);
        if (!$success) {
    ...
    public function processSendEmail()
    {
        ...
        $mail = new Mail($this->session->use_smtp, $this->session->smtp_server, $this->session->smtp_login, $this->session->smtp_password, $this->session->smtp_port, $this->session->smtp_encryption, $this->session->admin_email);
        if (file_exists(_PS_INSTALL_LANGS_PATH_ . $this->language->getLanguageIso() . '/mail_identifiers.txt')) {
            ...
            $content = file_get_contents(_PS_INSTALL_LANGS_PATH_ . Languages::DEFAULT_ISO . '/mail_identifiers.txt');
        }
        $vars = array('{firstname}' => $this->session->admin_firstname, '{lastname}' => $this->session->admin_lastname, '{shop_name}' => $this->session->shop_name, '{passwd}' => $this->session->admin_password, '{email}' => $this->session->admin_email, '{shop_url}' => Tools::getHttpHost(true) . __PS_BASE_URI__);
        $content = str_replace(array_keys($vars), array_values($vars), $content);
        $mail->send($this->l('%s - Login information', $this->session->shop_name), $content);
