...
use Prestashop\Installer\Model\Database;
use Prestashop\Tools;

/**
 * Step 3 : configure database and email connection
 */
class InstallControllerHttpDatabase extends ControllerHttp
{
    /**
    ...
    {
        $this->errors = $this->model_database->testDatabaseSettings($this->session->database_server, $this->session->database_name, $this->session->database_login, $this->session->database_password, $this->session->database_prefix, $this->session->step == 'process' ? true : $this->session->database_clear);
