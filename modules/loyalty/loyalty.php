...
use Prestashop\Order\OrderReturn;
use Prestashop\Order\Order;
use Prestashop\Customer;
use Prestashop\Tab;
use Prestashop\Order\OrderDetail;
...
class Loyalty extends Module
{
    ...
    {
        if (!parent::uninstall() || !$this->uninstallDB() || !Configuration::deleteByName('PS_LOYALTY_POINT_VALUE') || !Configuration::deleteByName('PS_LOYALTY_POINT_RATE') || !Configuration::deleteByName('PS_LOYALTY_NONE_AWARD') || !Configuration::deleteByName('PS_LOYALTY_MINIMAL') || !Configuration::deleteByName('PS_LOYALTY_VOUCHER_CATEGORY') || !Configuration::deleteByName('PS_LOYALTY_VOUCHER_DETAILS')) {
            return false;
        }
        return true;
    }
    ...
    private function _postProcess()
    {
        if (Tools::isSubmit('submitLoyalty')) {
            ...
            if (!count($this->_errors)) {
                ...
                Configuration::updateValue('PS_LOYALTY_POINT_VALUE', (double) Tools::getValue('point_value'));
                Configuration::updateValue('PS_LOYALTY_POINT_RATE', (double) Tools::getValue('point_rate'));
                Configuration::updateValue('PS_LOYALTY_NONE_AWARD', (int) Tools::getValue('PS_LOYALTY_NONE_AWARD'));
                Configuration::updateValue('PS_LOYALTY_MINIMAL', (double) Tools::getValue('minimal'));
    ...
    public function hookShoppingCart($params)
    {
        ...
        if (Validate::isLoadedObject($params['cart'])) {
            ...
            $this->smarty->assign(array('points' => (int) $points, 'voucher' => LoyaltyModule::getVoucherValue((int) $points), 'guest_checkout' => (int) Configuration::get('PS_GUEST_CHECKOUT_ENABLED')));
        } else {
    ...
    {
        $translations = array('Awaiting validation' => $this->l('Awaiting validation'), 'Available' => $this->l('Available'), 'Cancelled' => $this->l('Cancelled'), 'Already converted' => $this->l('Already converted'), 'Unavailable on discounts' => $this->l('Unavailable on discounts'), 'Not available on discounts.' => $this->l('Not available on discounts.'));
