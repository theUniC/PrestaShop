...
use Prestashop\Installer\Language;
...
class BlockLayered extends Module
{
            ...
            switch ($filter['type']) {
                case 'id_attribute_group':
                    ...
                    foreach ($attributes as $attribute) {
                        ...
                        $attribute_values_by_lang[$attribute['id_lang']]['c' . $attribute['id_name']][] = array('name' => !empty($attribute['name_url_name']) ? $attribute['name_url_name'] : $attribute['name'], 'id_name' => 'c' . $attribute['id_name'], 'value' => !empty($attribute['value_url_name']) ? $attribute['value_url_name'] : $attribute['value'], 'id_value' => $attribute['id_name'] . '_' . $attribute['id_value'], 'id_id_value' => $attribute['id_value'], 'category_name' => $filter['link_rewrite'], 'type' => $filter['type']);
                    }
                    break;
                case 'id_feature':
                    ...
                    foreach ($features as $feature) {
                        ...
                        $attribute_values_by_lang[$feature['id_lang']]['f' . $feature['id_name']][] = array('name' => !empty($feature['name_url_name']) ? $feature['name_url_name'] : $feature['name'], 'id_name' => 'f' . $feature['id_name'], 'value' => !empty($feature['value_url_name']) ? $feature['value_url_name'] : $feature['value'], 'id_value' => $feature['id_name'] . '_' . $feature['id_value'], 'id_id_value' => $feature['id_value'], 'category_name' => $filter['link_rewrite'], 'type' => $filter['type']);
                    }
                    break;
                case 'category':
                    ...
                    foreach ($categories as $category) {
                        ...
                        $attribute_values_by_lang[$category['id_lang']]['category'][] = array('name' => $this->translateWord('Categories', $category['id_lang']), 'id_name' => null, 'value' => $category['name'], 'id_value' => $category['id_category'], 'category_name' => $filter['link_rewrite'], 'type' => $filter['type']);
                    }
                    break;
                case 'manufacturer':
                    ...
                    foreach ($manufacturers as $manufacturer) {
                        ...
                        $attribute_values_by_lang[$manufacturer['id_lang']]['manufacturer'][] = array('name' => $this->translateWord('Manufacturer', $manufacturer['id_lang']), 'id_name' => null, 'value' => $manufacturer['name'], 'id_value' => $manufacturer['id_manufacturer'], 'category_name' => $filter['link_rewrite'], 'type' => $filter['type']);
                    }
                    break;
                case 'quantity':
                    $avaibility_list = array($this->translateWord('Not available', (int) $filter['id_lang']), $this->translateWord('In stock', (int) $filter['id_lang']));
                    foreach ($avaibility_list as $key => $quantity) {
                        $attribute_values_by_lang[$filter['id_lang']]['quantity'][] = array('name' => $this->translateWord('Availability', (int) $filter['id_lang']), 'id_name' => null, 'value' => $quantity, 'id_value' => $key, 'id_id_value' => 0, 'category_name' => $filter['link_rewrite'], 'type' => $filter['type']);
                    }
                    break;
                case 'condition':
                    $condition_list = array('new' => $this->translateWord('New', (int) $filter['id_lang']), 'used' => $this->translateWord('Used', (int) $filter['id_lang']), 'refurbished' => $this->translateWord('Refurbished', (int) $filter['id_lang']));
                    foreach ($condition_list as $key => $condition) {
                        $attribute_values_by_lang[$filter['id_lang']]['condition'][] = array('name' => $this->translateWord('Condition', (int) $filter['id_lang']), 'id_name' => null, 'value' => $condition, 'id_value' => $key, 'category_name' => $filter['link_rewrite'], 'type' => $filter['type']);
                    }
                    break;
            }
        }
    ...
    public function translateWord($string, $id_lang)
    {
        ...
        if (isset($_MODULES[$id_lang][$current_key])) {
            ...
            if (isset($_MODULES[$id_lang][Tools::strtolower($current_key)])) {
                $ret = stripslashes($_MODULES[$id_lang][Tools::strtolower($current_key)]);
            } else {
                if (isset($_MODULES[$id_lang][$default_key])) {
                    $ret = stripslashes($_MODULES[$id_lang][$default_key]);
                } else {
                    if (isset($_MODULES[$id_lang][Tools::strtolower($default_key)])) {
    ...
    public function hookAfterSaveFeature($params)
    {
        ...
        Db::getInstance()->execute('DELETE FROM ' . _DB_PREFIX_ . 'layered_indexable_feature_lang_value WHERE id_feature = ' . (int) $params['id_feature']);
        // don't care about the id_lang
    ...
    public function hookFeatureValueForm($params)
    {
        ...
        $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('SELECT url_name, meta_title, id_lang FROM ' . _DB_PREFIX_ . 'layered_indexable_feature_value_lang_value
    ...
    public function hookAttributeForm($params)
    {
        ...
        $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('SELECT url_name, meta_title, id_lang FROM ' . _DB_PREFIX_ . 'layered_indexable_attribute_lang_value
    ...
    public function hookAttributeGroupForm($params)
    {
        ...
        $lang_value = array();
        $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('SELECT url_name, meta_title, id_lang FROM ' . _DB_PREFIX_ . 'layered_indexable_attribute_group_lang_value
		WHERE id_attribute_group = ' . (int) $params['id_attribute_group']);
        if ($result) {
            foreach ($result as $data) {
                $lang_value[$data['id_lang']] = array('url_name' => $data['url_name'], 'meta_title' => $data['meta_title']);
    ...
    public function hookFeatureForm($params)
    {
        ...
        $lang_value = array();
        $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('SELECT url_name, meta_title, id_lang FROM ' . _DB_PREFIX_ . 'layered_indexable_feature_lang_value
    ...
    public function hookFooter($params)
    {
        ...
        if (basename($_SERVER['PHP_SELF']) == 'category.php' && version_compare(_PS_VERSION_, '1.5', '<') || version_compare(_PS_VERSION_, '1.5', '>') && Dispatcher::getInstance()->getController() == 'category') {
            return '
			<script type="text/javascript">
				//<![CDATA[
...
' . $sql_query['group']);
    ...
    private static function getWeightFilterSubQuery($filter_value, $ignore_join)
    {
        if (isset($filter_value) && $filter_value) {
            if ($filter_value[0] != 0 || $filter_value[1] != 0) {
                return array('where' => ' AND p.`weight` BETWEEN ' . (double) ($filter_value[0] - 0.001) . ' AND ' . (double) ($filter_value[1] + 0.001) . ' ');
            }
        }
        return array();
    }
    ...
    public function ajaxCall()
    {
        ...
        return Tools::jsonEncode(array('filtersBlock' => utf8_encode($this->generateFiltersBlock($selected_filters)), 'productList' => utf8_encode($product_list), 'pagination' => $smarty->fetch(_PS_THEME_DIR_ . 'pagination.tpl'), 'categoryCount' => $category_count));
    }
    ...
    public function rebuildLayeredCache($products_ids = array(), $categories_ids = array())
    {
		...
		WHERE c.active = 1' . (count($categories_ids) ? ' AND cp.id_category IN (' . implode(',', $categories_ids) . ')' : '') . ' AND ' . $alias . '.active = 1' . (count($products_ids) ? ' AND p.id_product IN (' . implode(',', $products_ids) . ')' : '') . ' AND (fv.custom IS NULL OR fv.custom = 0)
		GROUP BY p.id_product');
        if (version_compare(_PS_VERSION_, '1.5', '>')) {
            $shop_list = Shop::getShops(false, null, true);
        } else {
