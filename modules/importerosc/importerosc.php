...
class importerosc extends ImportModule
{
    ...
    {
        $identifier = 'id_state';
        $states = array(0 => array('id_state' => 0, 'id_country' => 0, 'id_zone' => 0, 'iso_code' => 999, 'name' => 'osc', 'active' => 0));
        return $this->autoFormat($states, $identifier);
    }
    public function getGroups()
    {
        $idLang = $this->getDefaultIdLang();
        return array(1 => array('id_group' => 1, 'price_display_method' => 0, 'name' => array($idLang => $this->l('Default osCommerce Group'))));
    }
    ...
    public function getOrdersStates($limit = 0, $nrb_import = 100)
    {
        ...
        $ordersStates = $this->executeS('
									...
									LIMIT ' . (int) $limit . ' , ' . (int) $nrb_import);
        //IF(`public_flag` = 0, 1, 0) as hidden
