...
class ProductCommentCriterion
{
    ...
    {
        if (!Validate::isUnsignedId($id_product_comment_criterion) || !Validate::isUnsignedId($id_product)) {
            die(Tools::displayError());
        }
        return Db::getInstance()->execute('
    ...
    {
        if (!Validate::isUnsignedId($id_product_comment) || !Validate::isUnsignedId($id_product_comment_criterion)) {
            die(Tools::displayError());
        }
        if ($grade < 0) {
            $grade = 0;
