...
">
    ...
    public function hookProductTab($params)
    {
        require_once dirname(__FILE__) . '/ProductComment.php';
        require_once dirname(__FILE__) . '/ProductCommentCriterion.php';
        $this->context->smarty->assign(array('allow_guests' => (int) Configuration::get('PRODUCT_COMMENTS_ALLOW_GUESTS'), 'comments' => ProductComment::getByProduct((int) Tools::getValue('id_product')), 'criterions' => ProductCommentCriterion::getByProduct((int) Tools::getValue('id_product'), $this->context->language->id), 'nbComments' => (int) ProductComment::getCommentNumber((int) Tools::getValue('id_product'))));
    ...
    public function hookproductOutOfStock($params)
    {
        ...
        $this->context->smarty->assign(array('id_product_comment_form' => (int) Tools::getValue('id_product'), 'product' => new Product((int) Tools::getValue('id_product'), false, $this->context->language->id), 'secure_key' => $this->secure_key, 'logged' => (int) $this->context->customer->isLogged(true), 'allow_guests' => (int) Configuration::get('PRODUCT_COMMENTS_ALLOW_GUESTS'), 'productcomment_cover' => (int) Tools::getValue('id_product') . '-' . (int) $image['id_image'], 'mediumSize' => Image::getSize(ImageType::getFormatedName('medium')), 'criterions' => ProductCommentCriterion::getByProduct((int) Tools::getValue('id_product'), $this->context->language->id), 'action_url' => '', 'averageTotal' => (int) $average['grade'], 'too_early' => $customerComment && strtotime($customerComment['date_add']) + Configuration::get('PRODUCT_COMMENTS_MINIMAL_TIME') > time(), 'nbComments' => (int) ProductComment::getCommentNumber((int) Tools::getValue('id_product'))));
