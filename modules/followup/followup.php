...
class Followup extends Module
{
    public function __construct()
    {
        ...
        $this->confKeys = array('PS_FOLLOW_UP_ENABLE_1', 'PS_FOLLOW_UP_ENABLE_2', 'PS_FOLLOW_UP_ENABLE_3', 'PS_FOLLOW_UP_ENABLE_4', 'PS_FOLLOW_UP_AMOUNT_1', 'PS_FOLLOW_UP_AMOUNT_2', 'PS_FOLLOW_UP_AMOUNT_3', 'PS_FOLLOW_UP_AMOUNT_4', 'PS_FOLLOW_UP_DAYS_1', 'PS_FOLLOW_UP_DAYS_2', 'PS_FOLLOW_UP_DAYS_3', 'PS_FOLLOW_UP_DAYS_4', 'PS_FOLLOW_UP_THRESHOLD_3', 'PS_FOLLOW_UP_DAYS_THRESHOLD_4', 'PS_FOLLOW_UP_CLEAN_DB');
        parent::__construct();
    ...
    private function cancelledCart($count = false)
    {
        ...
        foreach ($emails as $email) {
            $voucher = $this->createDiscount(1, (double) $conf['PS_FOLLOW_UP_AMOUNT_1'], (int) $email['id_customer'], strftime('%Y-%m-%d', strtotime('+' . (int) $conf['PS_FOLLOW_UP_DAYS_1'] . ' day')), $this->l('Discount for your cancelled cart'));
            if ($voucher !== false) {
    ...
    private function reOrder($count = false)
    {
        ...
        foreach ($emails as $email) {
            $voucher = $this->createDiscount(2, (double) $conf['PS_FOLLOW_UP_AMOUNT_2'], (int) $email['id_customer'], strftime('%Y-%m-%d', strtotime('+' . (int) $conf['PS_FOLLOW_UP_DAYS_2'] . ' day')), $this->l('Thank you for your order.'));
            if ($voucher !== false) {
    ...
    private function bestCustomer($count = false)
    {
		...
		GROUP BY o.id_customer
			HAVING total >= ' . (double) Configuration::get('PS_FOLLOW_UP_THRESHOLD_3');
        $emails = Db::getInstance()->executeS($sql);
        if ($count or !sizeof($emails)) {
            return sizeof($emails);
        }
        ...
        foreach ($emails as $email) {
            $voucher = $this->createDiscount(3, (double) $conf['PS_FOLLOW_UP_AMOUNT_3'], (int) $email['id_customer'], strftime('%Y-%m-%d', strtotime('+' . (int) $conf['PS_FOLLOW_UP_DAYS_3'] . ' day')), $this->l('You are one of our best customers!'));
            if ($voucher !== false) {
    ...
    private function badCustomer($count = false)
    {
        ...
        foreach ($emails as $email) {
            $voucher = $this->createDiscount(4, (double) $conf['PS_FOLLOW_UP_AMOUNT_4'], (int) $email['id_customer'], strftime('%Y-%m-%d', strtotime('+' . (int) $conf['PS_FOLLOW_UP_DAYS_4'] . ' day')), $this->l('We miss you!'));
            if ($voucher !== false) {
