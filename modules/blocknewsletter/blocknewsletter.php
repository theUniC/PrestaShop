...
class Blocknewsletter extends Module
{
    ...
    public function __construct()
    {
        ...
        $this->error = false;
        $this->valid = false;
        $this->_files = array('name' => array('newsletter_conf', 'newsletter_voucher'), 'ext' => array(0 => 'html', 1 => 'txt'));
    }
    public function install()
    {
        if (parent::install() == false || $this->registerHook('leftColumn') == false || $this->registerHook('header') == false || $this->registerHook('actionCustomerAccountAdd') == false) {
            return false;
        }
            ...
            if ($_POST['action'] == '1') {
                ...
                if ($register_status < 1) {
                    ...
                    if ($register_status == self::GUEST_REGISTERED) {
                        ...
                        if ($register_status == self::CUSTOMER_REGISTERED) {
    ...
    private function _prepareHook($params)
    {
        if (Tools::isSubmit('submitNewsletter')) {
            ...
            if ($this->error) {
                $this->smarty->assign(array('color' => 'red', 'msg' => $this->error, 'nw_value' => isset($_POST['email']) ? pSQL($_POST['email']) : false, 'nw_error' => true, 'action' => $_POST['action']));
            } else {
                if ($this->valid) {
                    $this->smarty->assign(array('color' => 'green', 'msg' => $this->valid, 'nw_error' => false));
                }
            }
        }
