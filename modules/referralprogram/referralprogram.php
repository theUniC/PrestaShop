...
use \ReferralProgramModule;
use Prestashop\CartRule;
use \ReferralProgram;
use Prestashop\Rijndael;
use Prestashop\Blowfish;
...
class ReferralProgram extends Module
{
    ...
    public function install()
    {
        ...
        if (!parent::install() or !$this->installDB() or !Configuration::updateValue('REFERRAL_DISCOUNT_DESCRIPTION', $desc) or !Configuration::updateValue('REFERRAL_ORDER_QUANTITY', 1) or !Configuration::updateValue('REFERRAL_DISCOUNT_TYPE', 2) or !Configuration::updateValue('REFERRAL_NB_FRIENDS', 5) or !$this->registerHook('shoppingCart') or !$this->registerHook('orderConfirmation') or !$this->registerHook('updateOrderStatus') or !$this->registerHook('adminCustomers') or !$this->registerHook('createAccount') or !$this->registerHook('createAccountForm') or !$this->registerHook('customerAccount')) {
            return false;
        }
    ...
    public function uninstall()
    {
        ...
        if (!parent::uninstall() or !$this->uninstallDB() or !$this->removeMail() or !$result or !Configuration::deleteByName('REFERRAL_PERCENTAGE') or !Configuration::deleteByName('REFERRAL_ORDER_QUANTITY') or !Configuration::deleteByName('REFERRAL_DISCOUNT_TYPE') or !Configuration::deleteByName('REFERRAL_NB_FRIENDS') or !Configuration::deleteByName('REFERRAL_DISCOUNT_DESCRIPTION')) {
            return false;
        }
        return true;
    }
    ...
    {
        if ((double) $discountValue and (int) $discountType) {
            if ($discountType == 1) {
    ...
    private function _postProcess()
    {
        ...
        foreach (Tools::getValue('discount_value') as $id_currency => $discount_value) {
            Configuration::updateValue('REFERRAL_DISCOUNT_VALUE_' . (int) $id_currency, (double) $discount_value);
        }
    ...
    {
        $forbiddenKey = array('submitUpdate');
        // Forbidden key
