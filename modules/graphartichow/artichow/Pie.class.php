...
class awPie extends awComponent
{
    ...
    public function drawComponent(awDriver $driver, $x1, $y1, $x2, $y2, $aliasing)
    {
        ...
        if ($aliasing) {
            ...
            $driver = $image->getDriver($width / $image->width, $height / $image->height, ($width / 2 + $side->left) / $image->width, ($height / 2 + $side->top) / $image->height);
        }
        // Draw 3D effect
        ...
        if ($aliasing) {
            ...
            $mainDriver->copyResizeImage($image, new awPoint($x1 - $side->left / 2, $y1 - $side->top / 2), new awPoint($x1 - $side->left / 2 + $image->width / 2, $y1 - $side->top / 2 + $image->height / 2), new awPoint(0, 0), new awPoint($image->width, $image->height), TRUE);
            $driver = $mainDriver;
        }
        // Get labels values
        $pc = array();
