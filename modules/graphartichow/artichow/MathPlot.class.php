...
class awMathFunction implements awLegendable
{
    ...
    {
        $this->f = (string) $f;
        $this->fromX = is_null($fromX) ? NULL : (double) $fromX;
        $this->toX = is_null($toX) ? NULL : (double) $toX;
        ...
        foreach ($this->functions as $function) {
            ...
            for ($i = $fromX; $i <= $toX; $i += $this->interval) {
                ...
                if ($old !== NULL) {
                    ...
                    if ($line->p1->y >= $y1 and $line->p1->y <= $y2 or $line->p2->y >= $y1 and $line->p2->y <= $y2) {
                        $driver->line($function->getColor(), $line);
                    }
                }
                $old = $p;
            }
