...
class awAntiSpam extends awImage
{
    ...
    public function check($qName, $value, $case = TRUE)
    {
        $this->session();
        $session = 'artichow_' . (string) $qName;
        return array_key_exists($session, $_SESSION) === TRUE and $case ? strtolower($_SESSION[$session]) === strtolower((string) $value) : $_SESSION[$session] === (string) $value;
    }
    /**
     * Draw image
     */
    ...
    protected function drawNoise($width, $height)
    {
        ...
        for ($i = 0; $i < $points; $i++) {
            $this->driver->point($color, new awPoint(mt_rand(0, $width), mt_rand(0, $height)));
        }
    }
