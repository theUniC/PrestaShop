 ...
 *
 */
if (is_file(dirname(__FILE__) . '/Artichow.cfg.php')) {
    // For PHP 4+5 version
...
class awImage
{
    ...
    public function drawComponent(awComponent $component)
    {
        ...
        $driver->setImageSize($this->width - $shadow->left - $shadow->right - $border * 2, $this->height - $shadow->top - $shadow->bottom - $border * 2);
        ...
        if ($component->top !== NULL and $component->left !== NULL) {
            $driver->setAbsPosition($border + $shadow->left + $component->left, $border + $shadow->top + $component->top);
        } else {
    ...
    protected function drawShadow()
    {
        ...
        $this->shadow->draw($driver, new awPoint(0, 0), new awPoint($this->width, $this->height), awShadow::IN);
    }
    /**
