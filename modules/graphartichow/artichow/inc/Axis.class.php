...
class awAxis
{
     ...
     * @var array
     */
    protected $rangeCallback = array('toValue' => 'toProportionalValue', 'toPosition' => 'toProportionalPosition');
    /**
     * Build the axis
     *
    ...
    {
        $this->titlePosition = (double) $position;
    }
    /**
    ...
    {
        if ($min !== NULL) {
            $this->range[0] = (double) $min;
        }
        if ($max !== NULL) {
            $this->range[1] = (double) $max;
        }
    }
    /**
     * Get axis range
    ...
    {
        $this->rangeCallback = array('toValue' => (string) $toValue, 'toPosition' => (string) $toPosition);
    }
    /**
    ...
    protected function getPointFromPosition($position)
    {
        ...
        $size = $vector->getSize();
        return $vector->p1->move(cos($angle) * $size * $position, -1 * sin($angle) * $size * $position);
    }
    /**
     * Draw axis
     *
    ...
    public function autoScale()
    {
        ...
        if ($intervalNormalize <= 1.5) {
            ...
            if ($intervalNormalize <= 2) {
                ...
                if ($intervalNormalize <= 3) {
                    $intervalReal = 3;
                    $labelNumber = 4;
                } else {
                    if ($intervalNormalize <= 4) {
                        $intervalReal = 4;
                        $labelNumber = 5;
                    } else {
                        if ($intervalNormalize <= 5) {
                            $intervalReal = 5;
                            $labelNumber = 6;
                        } else {
                            if ($intervalNormalize <= 8) {
                                $intervalReal = 8;
                                $labelNumber = 5;
                            } else {
                                if ($intervalNormalize <= 10) {
                                    $intervalReal = 10;
                                    $labelNumber = 6;
                                }
                            }
