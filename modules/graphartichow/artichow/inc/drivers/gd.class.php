...
use \awPoint;
use \awLine;
use \awGDGradientDriver;
use \awPolygon;
use \Driver;
use \Point;
use \Line;
use \awLinearGradient;
use \awColor;
/*
...
class awGDDriver extends Driver
{
    ...
    public function copyResizeImage(awImage $image, awPoint $d1, awPoint $d2, awPoint $s1, awPoint $s2, $resample = TRUE)
    {
        ...
        $driver = $image->getDriver();
        $function($this->resource, $driver->resource, $this->x + $d1->x, $this->y + $d1->y, $s1->x, $s1->y, $d2->x - $d1->x, $d2->y - $d1->y, $s2->x - $s1->x, $s2->y - $s1->y);
    }
    ...
    {
        imagefilledarc($this->resource, $this->x + $center->x, $this->y + $center->y, $width, $height, $from, $to, $this->getColor($color), IMG_ARC_EDGED | IMG_ARC_NOFILL);
    }
    public function filledArc(awColor $color, awPoint $center, $width, $height, $from, $to)
    {
        imagefilledarc($this->resource, $this->x + $center->x, $this->y + $center->y, $width, $height, $from, $to, $this->getColor($color), IMG_ARC_PIE);
    }
    public function ellipse(awColor $color, awPoint $center, $width, $height)
    {
        list($x, $y) = $center->getLocation();
        $rgb = $this->getColor($color);
        imageellipse($this->resource, $this->x + $x, $this->y + $y, $width, $height, $rgb);
    }
    public function filledEllipse($background, awPoint $center, $width, $height)
    {
        if ($background instanceof awColor) {
            ...
            imagefilledellipse($this->resource, $this->x + $x, $this->y + $y, $width, $height, $rgb);
        } else {
...
class awGDGradientDriver
{
    ...
    private function drawFilledTriangleVertically(awGradient $gradient, awPolygon $polygon)
    {
        ...
        for ($y = round($yMin); $y < round($yMax); $y++) {
            ...
            if (count($toX) === 1) {
                $fillLine = new Line(new Point($fromX, $y), new Point($toX[0], $y));
            } else {
    ...
    {
        return new awColor($this->getRed($pos), $this->getGreen($pos), $this->getBlue($pos), $this->getAlpha($pos));
    }
