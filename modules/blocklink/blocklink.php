...
class BlockLink extends Module
{
    ...
    {
        if (!parent::install() || !$this->registerHook('leftColumn') || !$this->registerHook('header') || !Db::getInstance()->execute('
			...
			ENGINE=' . _MYSQL_ENGINE_ . ' default CHARSET=utf8') || !Configuration::updateValue('PS_BLOCKLINK_TITLE', array('1' => 'Block link', '2' => 'Bloc lien'))) {
            return false;
        }
        return true;
    }
    ...
    {
        $links = $this->getLinks();
        $this->smarty->assign(array('blocklink_links' => $links, 'title' => Configuration::get('PS_BLOCKLINK_TITLE', $this->context->language->id), 'url' => Configuration::get('PS_BLOCKLINK_URL'), 'lang' => 'text_' . $this->context->language->id));
        if (!$links) {
            return false;
        }
    ...
    public function addLink()
    {
        ...
        if ($id_link = Tools::getValue('id_link')) {
            ...
            if (!Db::getInstance()->execute('INSERT INTO ' . _DB_PREFIX_ . 'blocklink 
														VALUES (NULL, \'' . pSQL($_POST['url']) . '\', ' . ((isset($_POST['newWindow']) && $_POST['newWindow']) == 'on' ? 1 : 0) . ')') || !($id_link = Db::getInstance()->Insert_ID())) {
                return false;
            }
            foreach ($languages as $language) {
                if (!empty($_POST['text_' . $language['id_lang']])) {
        ...
        if (!Shop::isFeatureActive()) {
            Db::getInstance()->insert('blocklink_shop', array('id_blocklink' => (int) $id_link, 'id_shop' => (int) Context::getContext()->shop->id));
        } else {
            ...
            foreach ($assos_shop as $id_shop => $row) {
                Db::getInstance()->insert('blocklink_shop', array('id_blocklink' => (int) $id_link, 'id_shop' => (int) $id_shop));
            }
        }
        return true;
    }
    ...
    {
        return Db::getInstance()->execute('DELETE FROM ' . _DB_PREFIX_ . 'blocklink WHERE `id_blocklink` = ' . (int) $_GET['id']) && Db::getInstance()->execute('DELETE FROM ' . _DB_PREFIX_ . 'blocklink_shop WHERE `id_blocklink` = ' . (int) $_GET['id']) && Db::getInstance()->execute('DELETE FROM ' . _DB_PREFIX_ . 'blocklink_lang WHERE `id_blocklink` = ' . (int) $_GET['id']);
    }
