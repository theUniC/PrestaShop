...
class CrossSelling extends Module
{
    ...
    {
        if (!parent::install() or !$this->registerHook('productFooter') or !$this->registerHook('header') or !$this->registerHook('shoppingCart') or !$this->registerHook('actionOrderStatusPostUpdate') or !Configuration::updateValue('CROSSSELLING_DISPLAY_PRICE', 0) or !Configuration::updateValue('CROSSSELLING_NBR', 10)) {
            return false;
        }
    ...
    {
        $this->_clearCache('crossselling.tpl');
        if (!parent::uninstall() or !Configuration::deleteByName('CROSSSELLING_DISPLAY_PRICE') or !Configuration::deleteByName('CROSSSELLING_NBR')) {
            return false;
        }
        return true;
    }
    ...
    public function hookshoppingCart($params)
    {
        ...
        if (!$this->isCached('crossselling.tpl', $this->getCacheId($cache_id))) {
            ...
            if (sizeof($orders)) {
                ...
                $this->smarty->assign(array('order' => count($pIds) > 1 ? true : false, 'orderProducts' => $orderProducts, 'middlePosition_crossselling' => round(sizeof($orderProducts) / 2, 0), 'crossDisplayPrice' => Configuration::get('CROSSSELLING_DISPLAY_PRICE')));
            }
        }
    ...
    public function hookProductFooter($params)
    {
        ...
        if (!$this->isCached('crossselling.tpl', $this->getCacheId($cache_id))) {
            ...
            if (sizeof($orders)) {
                ...
                $this->smarty->assign(array('order' => false, 'orderProducts' => $orderProducts, 'middlePosition_crossselling' => round(sizeof($orderProducts) / 2, 0), 'crossDisplayPrice' => Configuration::get('CROSSSELLING_DISPLAY_PRICE')));
            }
        }
        return $this->display(__FILE__, 'crossselling.tpl', $this->getCacheId($cache_id));
    }
