...
use Prestashop\Module\ImportModule;
/*
            ...
            if (!array_key_exists('hidden', $import)) {
                $html .= '<label>' . $import['name'] . ' : </label>
									<div class="margin-form">
										...
										<input type="checkbox" class="truncateTable" id="' . $key . '" name="delete_' . $import['className'] . '">' : '') . (array_key_exists('hasImage', $import) ? '
										<label class="t"><img src="../img/admin/picture.gif" alt="Images" title="Images"></label>
										<input type="checkbox" class="importImages" id="' . $key . '" name="images_' . $import['className'] . '">' : '') . (array_key_exists('info', $import) ? '<p>' . $import['info'] . '</p>' : '') . '
									</div>';
            }
        }
        $html .= '</div><hr>
    ...
    private function replaceForeignKey(&$item, $table)
    {
        ...
        foreach ($foreingKey as $key) {
            if ($table == 'product' and $key == 'id_category') {
                ...
                if ($table == 'customer' and $key == 'id_group') {
        ...
        foreach ($rules['required'] as $field) {
            if (($value = $fields[$field]) == false and (string) $value != '0') {
                if ($hasErrors == 2) {
                    ...
                    $returnErrors[] = sprintf($this->l('The field %s is required.'), call_user_func(array($className, 'displayFieldName'), $field, $className));
                }
            }
        }
            ...
            if (array_key_exists($field, $fields) and $field != 'passwd') {
                if ($fields[$field] !== false and Tools::strlen($fields[$field]) > $maxLength) {
                    if ($hasErrors == 2) {
                        ...
                        $returnErrors[] = sprintf($this->l('The field %1$s is too long (%2$d chars max).'), call_user_func(array($className, 'displayFieldName'), $field, $className), $maxLength);
                    }
                }
            }
        }
                ...
                if (($value = $fields[$field]) !== false and $field != 'passwd') {
                    if (!Validate::$function($value)) {
                        if ($hasErrors == 2) {
                            ...
                            $returnErrors[] = sprintf($this->l('The field %s is invalid.'), call_user_func(array($className, 'displayFieldName'), $field, $className));
                        }
                    }
                }
            }
            ...
            foreach ($rules['requiredLang'] as $fieldLang) {
                if (($empty = $fields[$fieldLang][$matchIdLang[$defaultLanguage->id]]) === false || empty($empty)) {
                    if ($hasErrors == 2) {
                        ...
                        $returnErrors[] = sprintf($this->l('This field %1$s is required at least in %2$s'), call_user_func(array($className, 'displayFieldName'), $fieldLang, $className), $defaultLanguage->name);
                    }
                }
            }
                ...
                foreach ($languages as $language) {
                    if (isset($fields[$fieldLang][$language['id_lang']]) && $fields[$fieldLang] !== false and Tools::strlen($fields[$fieldLang][$language['id_lang']]) > $maxLength) {
                        if ($hasErrors == 2) {
                            ...
                            $returnErrors[] = sprintf($this->l('This field %1$s (%2$s) is too long: %3$d chars max.'), call_user_func(array($className, 'displayFieldName'), $fieldLang, $className), $language['name'], $maxLength);
                        }
                    }
                }
            }
                    ...
                    if (array_key_exists($fieldLang, $fields) and array_key_exists($language['id_lang'], $fields[$fieldLang]) and ($value = $fields[$fieldLang][$language['id_lang']]) !== false and !empty($value)) {
                        if (!Validate::$function($value)) {
                            if ($hasErrors == 2) {
                                ...
                                $returnErrors[] = sprintf($this->l('The field %1$s (%2$s) is invalid.'), call_user_func(array($className, 'displayFieldName'), $fieldLang, $className), $language['name']);
                            }
                        }
                    }
                }
