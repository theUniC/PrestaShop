...
class AdminThemesController extends AdminController
{
    ...
    protected function checkMobileNeeds()
    {
        ...
        if (!$paypal_installed && in_array($iso_code, $paypal_countries)) {
            if (!$this->isXmlHttpRequest()) {
                $this->warnings[] = $this->l('At this time, the mobile theme only works with PayPal\'s payment module. Please activate and configure the PayPal module to enable mobile payments.') . '<br>' . $this->l('In order to use the mobile theme, you must install and configure the PayPal module.');
            }
        }
    }
    public function renderForm()
    {
        ...
        if ($this->display == 'add' || !$this->object->id) {
            ...
            $this->fields_form['input'][] = array('type' => 'select', 'name' => 'based_on', 'label' => $this->l('Copy missing files from existing theme:'), 'desc' => $this->l('If you create a new theme, it\'s recommended that you use default theme files.'), 'options' => array('id' => 'id', 'name' => 'name', 'default' => array('value' => 0, 'label' => '&nbsp;-&nbsp;'), 'query' => $theme_query));
        } else {
