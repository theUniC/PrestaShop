...
use Prestashop\Stock\SupplyOrderState;
...
use Prestashop\Stock\SupplyOrderReceiptHistory;
...
class AdminSupplyOrdersController extends AdminController
{
    ...
    public function getList($id_lang, $order_by = null, $order_way = null, $start = 0, $limit = null, $id_lang_shop = false)
    {
        ...
        if (!$this->is_template_list) {
            ...
            $this->toolbar_btn['export-csv-orders'] = array('short' => 'Export Orders', 'href' => $this->context->link->getAdminLink('AdminSupplyOrders') . '&amp;csv_orders&id_warehouse=' . $this->getCurrentWarehouse(), 'desc' => $this->l('Export Orders (CSV)'));
            $this->toolbar_btn['export-csv-details'] = array('short' => 'Export Orders Details', 'href' => $this->context->link->getAdminLink('AdminSupplyOrders') . '&amp;csv_orders_details&id_warehouse=' . $this->getCurrentWarehouse(), 'desc' => $this->l('Export Orders Details (CSV)'));
    ...
    public function renderList()
    {
        ...
        $this->initToolbar();
        unset($this->toolbar_btn['new']);
        $this->toolbar_btn['new'] = array('href' => self::$currentIndex . '&amp;add' . $this->table . '&mod=template&amp;token=' . $this->token, 'desc' => $this->l('Add new template'));
        // inits list
    ...
    public function initChangeStateContent()
    {
        ...
        $this->fields_form[]['form'] = array('legend' => array('title' => $this->l('Supply order status'), 'image' => '../img/admin/cms.gif'));
        ...
        $this->context->smarty->assign(array('content' => $content, 'url_post' => self::$currentIndex . '&token=' . $this->token));
    }
    /**
     * Init the content of change state action
     */
    ...
    public function initUpdateReceiptContent()
    {
        ...
        $helper->toolbar_btn = $this->toolbar_btn;
        $helper->ajax_params = array('display_product_history' => 1);
...
', implode(';', array_map(array('CSVCore', 'wrap'), $row)));
    ...
    public function displayUpdateReceiptLink($token = null, $id)
    {
        ...
        $this->context->smarty->assign(array('href' => self::$currentIndex . '&' . $this->identifier . '=' . $id . '&update_receipt&token=' . ($token != null ? $token : $this->token), 'action' => self::$cache_lang['Receipt']));
    ...
    public function displayChangestateLink($token = null, $id)
    {
        ...
        $this->context->smarty->assign(array('href' => self::$currentIndex . '&' . $this->identifier . '=' . $id . '&changestate&token=' . ($token != null ? $token : $this->token), 'action' => self::$cache_lang['State']));
    ...
    public function displayCreateSupplyOrderLink($token = null, $id)
    {
        ...
        $this->context->smarty->assign(array('href' => self::$currentIndex . '&' . $this->identifier . '=' . $id . '&create_supply_order&token=' . ($token != null ? $token : $this->token), 'confirm' => self::$cache_lang['CreateSupplyOrderConfirm'], 'action' => self::$cache_lang['CreateSupplyOrder']));
    ...
    public function ajaxProcess()
    {
        ...
        if (Tools::isSubmit('id') && !Tools::isSubmit('display_product_history')) {
            ...
            $this->fields_list = array('history_date' => array('title' => $this->l('Last update'), 'width' => 50, 'align' => 'left', 'type' => 'datetime', 'havingFilter' => true), 'history_employee' => array('title' => $this->l('Employee'), 'width' => 100, 'align' => 'left', 'havingFilter' => true), 'history_state_name' => array('title' => $this->l('Status'), 'width' => 100, 'align' => 'left', 'color' => 'color', 'havingFilter' => true));
            ...
            if (Tools::isSubmit('id') && Tools::isSubmit('display_product_history')) {
                ...
                unset($this->fields_list);
                $this->fields_list = array('date_add' => array('title' => $this->l('Last update'), 'width' => 50, 'align' => 'left', 'type' => 'datetime', 'havingFilter' => true), 'employee' => array('title' => $this->l('Employee'), 'width' => 100, 'align' => 'left', 'havingFilter' => true), 'quantity' => array('title' => $this->l('Quantity received'), 'width' => 100, 'align' => 'left', 'havingFilter' => true));
                // loads history of the given order
                unset($this->_select, $this->_join, $this->_where, $this->_orderBy, $this->_orderWay, $this->_group, $this->_filterHaving, $this->_filter);
