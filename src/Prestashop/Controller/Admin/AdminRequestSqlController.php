...
class AdminRequestSqlController extends AdminController
{
     ...
     */
    public static $encoding_file = array(array('value' => 1, 'name' => 'utf-8'), array('value' => 2, 'name' => 'iso-8859-1'));
    public function __construct()
    {
        ...
        $this->fields_list = array('id_request_sql' => array('title' => $this->l('ID'), 'width' => 25), 'name' => array('title' => $this->l('Name'), 'width' => 300), 'sql' => array('title' => $this->l('Request'), 'width' => 500));
        $this->fields_options = array('general' => array('title' => $this->l('Settings'), 'fields' => array('PS_ENCODING_FILE_MANAGER_SQL' => array('title' => $this->l('Select your encoding file by default'), 'cast' => 'intval', 'type' => 'select', 'identifier' => 'value', 'list' => self::$encoding_file, 'visibility' => Shop::CONTEXT_ALL)), 'submit' => array()));
    ...
    public function initToolbar()
    {
        if ($this->display == 'view' && ($id_request = Tools::getValue('id_request_sql'))) {
            $this->toolbar_btn['edit'] = array('href' => self::$currentIndex . '&amp;updaterequest_sql&amp;token=' . $this->token . '&amp;id_request_sql=' . (int) $id_request, 'desc' => $this->l('Edit this request'));
        }
        parent::initToolbar();
            ...
            switch ($key) {
                case 'checkedFrom':
                    if (isset($e[$key]['table'])) {
                        ...
                        if (isset($e[$key]['attribut'])) {
                            $this->errors[] = sprintf(Tools::displayError('The attribute "%1$s" does not exist in the table: %2$s.'), $e[$key]['attribut'][0], $e[$key]['attribut'][1]);
                        } else {
                ...
                case 'returnNameTable':
                    if (isset($e[$key]['reference'])) {
                        $this->errors[] = sprintf(Tools::displayError('The reference "%1$s" does not exist in the table: %2$s.'), $e[$key]['reference'][0], $e[$key]['attribut'][1]);
                    } else {
