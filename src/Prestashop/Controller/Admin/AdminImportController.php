...
use Prestashop\Stock\SupplyOrder;
use Prestashop\Stock\Warehouse;
use DateTime;
use Prestashop\Stock\SupplyOrderDetail;
...
class AdminImportController extends AdminController
{
    ...
    public function initToolbar()
    {
        switch ($this->display) {
            case 'import':
                ...
                $this->toolbar_btn['save-import'] = array('href' => '#', 'desc' => $this->l('Import .CSV data'));
                break;
        }
    }
    ...
    protected static function fillInfo($infos, $key, &$entity)
    {
        ...
        if (isset(self::$validators[$key][1]) && self::$validators[$key][1] == 'createMultiLangField' && Tools::getValue('iso_lang')) {
            ...
            if (!empty($infos) || $infos == '0') {
                // ($infos == '0') => if you want to disable a product by using "0" in active because empty('0') return true
    ...
    public function categoryImport()
    {
        ...
        for ($current_line = 0; $line = fgetcsv($handle, MAX_LINE_SIZE, $this->separator); $current_line++) {
            ...
            if (!$valid_link) {
                $this->warnings[] = sprintf(Tools::displayError('Rewrite link for %1$s (ID: %2$s) was re-written as %3$s.'), $bak, isset($info['id']) ? $info['id'] : 'null', $category->link_rewrite[$default_language_id]);
            }
            $res = false;
            if (($field_error = $category->validateFields(UNFRIENDLY_ERROR, true)) === true && ($lang_field_error = $category->validateFieldsLang(UNFRIENDLY_ERROR, true)) === true && empty($this->errors)) {
                $category_already_created = Category::searchByNameAndParentCategoryId($default_language_id, $category->name[$default_language_id], $category->id_parent);
    ...
    public function addressImport()
    {
        ...
        for ($current_line = 0; $line = fgetcsv($handle, MAX_LINE_SIZE, $this->separator); $current_line++) {
            ...
            if (isset($address->country) && is_numeric($address->country)) {
                ...
                if (isset($address->country) && is_string($address->country) && !empty($address->country)) {
            ...
            if (isset($address->state) && is_numeric($address->state)) {
                ...
                if (isset($address->state) && is_string($address->state) && !empty($address->state)) {
            ...
            if (isset($address->customer_email) && !empty($address->customer_email)) {
                if (Validate::isEmail($address->customer_email)) {
                    ...
                    if (count($customer_list) == 0) {
                        $this->errors[] = sprintf(Tools::displayError('%1$s does not exist in database %2$s (ID: %3$s) cannot be saved'), Db::getInstance()->getMsgError(), $address->customer_email, isset($info['id']) ? $info['id'] : 'null');
                    }
                } else {
                ...
                if (Customer::customerIdExistsStatic((int) $address->id_customer)) {
                    ...
                    if (count($customer_list) == 0) {
                        $this->errors[] = sprintf(Tools::displayError('%1$s does not exist in database %2$s (ID: %3$s) cannot be saved'), Db::getInstance()->getMsgError(), $customer->email, (int) $address->id_customer);
                    }
                } else {
            ...
            if (isset($address->manufacturer) && is_numeric($address->manufacturer) && Manufacturer::manufacturerExists((int) $address->manufacturer)) {
                ...
                if (isset($address->manufacturer) && is_string($address->manufacturer) && !empty($address->manufacturer)) {
                    $manufacturer = new Manufacturer();
                    $manufacturer->name = $address->manufacturer;
                    if (($field_error = $manufacturer->validateFields(UNFRIENDLY_ERROR, true)) === true && ($lang_field_error = $manufacturer->validateFieldsLang(UNFRIENDLY_ERROR, true)) === true && $manufacturer->add()) {
                        $address->id_manufacturer = (int) $manufacturer->id;
                    } else {
                        $this->errors[] = Db::getInstance()->getMsgError() . ' ' . sprintf(Tools::displayError('%1$s (ID: %2$s) cannot be saved'), $manufacturer->name, isset($manufacturer->id) ? $manufacturer->id : 'null');
                        $this->errors[] = ($field_error !== true ? $field_error : '') . (isset($lang_field_error) && $lang_field_error !== true ? $lang_field_error : '') . Db::getInstance()->getMsgError();
                    }
                }
            }
            if (isset($address->supplier) && is_numeric($address->supplier) && Supplier::supplierExists((int) $address->supplier)) {
                ...
                if (isset($address->supplier) && is_string($address->supplier) && !empty($address->supplier)) {
                    $supplier = new Supplier();
                    $supplier->name = $address->supplier;
                    if (($field_error = $supplier->validateFields(UNFRIENDLY_ERROR, true)) === true && ($lang_field_error = $supplier->validateFieldsLang(UNFRIENDLY_ERROR, true)) === true && $supplier->add()) {
                        $address->id_supplier = (int) $supplier->id;
                    } else {
                        $this->errors[] = Db::getInstance()->getMsgError() . ' ' . sprintf(Tools::displayError('%1$s (ID: %2$s) cannot be saved'), $supplier->name, isset($supplier->id) ? $supplier->id : 'null');
                        $this->errors[] = ($field_error !== true ? $field_error : '') . (isset($lang_field_error) && $lang_field_error !== true ? $lang_field_error : '') . Db::getInstance()->getMsgError();
                    }
                }
            }
            ...
            if (($field_error = $address->validateFields(UNFRIENDLY_ERROR, true)) === true && ($lang_field_error = $address->validateFieldsLang(UNFRIENDLY_ERROR, true)) === true) {
                if (isset($customer_list) && count($customer_list) > 0) {
                    ...
                    foreach ($customer_list as $customer) {
                        ...
                        $res = $address->add();
                        if (!$res) {
                            $this->errors[] = sprintf(Tools::displayError('%1$s (ID: %2$s) cannot be saved'), $info['alias'], isset($info['id']) ? $info['id'] : 'null');
                        }
                    }
                } else {
    ...
    public function manufacturerImport()
    {
        ...
        for ($current_line = 0; $line = fgetcsv($handle, MAX_LINE_SIZE, $this->separator); $current_line++) {
            ...
            if (!$res) {
                $this->errors[] = Db::getInstance()->getMsgError() . ' ' . sprintf(Tools::displayError('%1$s (ID: %2$s) cannot be saved'), isset($info['name']) ? Tools::safeOutput($info['name']) : 'No Name', isset($info['id']) ? Tools::safeOutput($info['id']) : 'No ID');
                $this->errors[] = ($field_error !== true ? $field_error : '') . (isset($lang_field_error) && $lang_field_error !== true ? $lang_field_error : '') . Db::getInstance()->getMsgError();
            }
        }
        $this->closeCsvFile($handle);
    ...
    public function supplyOrdersImport()
    {
        ...
        for ($current_line = 0; $line = fgetcsv($handle, MAX_LINE_SIZE, $this->separator); ++$current_line) {
            ...
            if (array_key_exists('id', $info) && (int) $info['id'] && SupplyOrder::exists((int) $info['id'])) {
                ...
                if (array_key_exists('reference', $info) && $info['reference'] && SupplyOrder::exists(pSQL($info['reference']))) {
            ...
            if (!Validate::isDateFormat($date_delivery_expected)) {
                $this->errors[] = sprintf($this->l('Date (%s) is not valid (at line %d). Format: %s.'), $date_delivery_expected, $current_line + 1, $this->l('YYYY-MM-DD'));
            } else {
                if (new DateTime($date_delivery_expected) <= new DateTime('yesterday')) {
                    $this->errors[] = sprintf($this->l('Date (%s) cannot be in the past (at line %d). Format: %s.'), $date_delivery_expected, $current_line + 1, $this->l('YYYY-MM-DD'));
                }
            }
