...
class AdminTaxesController extends AdminController
{
    public function __construct()
    {
        ...
        if (Configuration::get('PS_USE_ECOTAX')) {
            $this->fields_options['general']['fields']['PS_ECOTAX_TAX_RULES_GROUP_ID'] = array('title' => $this->l('Ecotax'), 'desc' => $this->l('Define the ecotax (e.g. French ecotax: 19.6%).'), 'cast' => 'intval', 'type' => 'select', 'identifier' => 'id_tax', 'identifier' => 'id_tax_rules_group', 'list' => TaxRulesGroup::getTaxRulesGroupsForOptions());
        }
        parent::__construct();
        ...
        if ($this->action == 'save') {
            ...
            if (!count($this->errors)) {
                ...
                if (isset($id) && !empty($id)) {
                    ...
                    } elseif (($_POST['id_' . $this->table] = $object->id) && $this->postImage($object->id) && $this->_redirect) {
