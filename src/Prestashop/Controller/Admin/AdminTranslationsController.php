...
use Prestashop\Theme;
use Prestashop\Module\Module;
use Prestashop\Autoload;
/*
...
class AdminTranslationsController extends AdminController
{
    ...
    public function initForm($method_name)
    {
        ...
        $this->tpl_view_vars = array('lang' => $this->lang_selected->iso_code, 'title' => $title, 'type' => $this->type_selected, 'theme' => $this->theme_selected, 'post_limit_exceeded' => $this->post_limit_exceed, 'url_submit' => self::$currentIndex . '&submitTranslations' . ucfirst($this->type_selected) . '=1&token=' . $this->token, 'toggle_button' => $this->displayToggleButton(), 'textarea_sized' => TEXTAREA_SIZED, 'auto_translate' => '');
    ...
    public function initToolbar()
    {
        ...
        $this->toolbar_btn['save'] = array('href' => '#', 'desc' => $this->l('Update translations'));
        $this->toolbar_btn['cancel'] = array('href' => self::$currentIndex . '&token=' . $this->token, 'desc' => $this->l('Cancel'));
    }
    /**
    ...
    protected function writeTranslationFile($override_file = false)
    {
        ...
        if (!file_exists($file_path)) {
            if (!file_exists(dirname($file_path)) && !mkdir(dirname($file_path), 511, true)) {
...
?>');
    ...
    public function submitCopyLang()
    {
        if (!($from_lang = Tools::getValue('fromLang')) || !($to_lang = Tools::getValue('toLang'))) {
            ...
            if (!($from_theme = Tools::getValue('fromTheme')) || !($to_theme = Tools::getValue('toTheme'))) {
                $this->errors[] = $this->l('You must select two themes in order to copy data from one to another.');
            } else {
                if (!Language::copyLanguageData(Language::getIdByIso($from_lang), Language::getIdByIso($to_lang))) {
...
', '?>', ''))) {
    ...
    public function submitAddLang()
    {
        ...
        if (Validate::isLangIsoCode($arr_import_lang[0])) {
            if ($content = Tools::file_get_contents('http://www.prestashop.com/download/lang_packs/gzip/' . $arr_import_lang[1] . '/' . Tools::strtolower($arr_import_lang[0]) . '.gzip', false, @stream_context_create(array('http' => array('method' => 'GET', 'timeout' => 5))))) {
    ...
    protected function findAndWriteTranslationsIntoFile($file_name, $files, $theme_name, $module_name, $dir = false)
    {
        ...
        if (!isset($cache_file[$theme_name . '-' . $file_name])) {
            ...
            if (!Tools::file_exists_cache(dirname($file_name))) {
                mkdir(dirname($file_name), 511, true);
            }
    ...
    public function clearModuleFiles($files, $type_clear = 'file', $path = '')
    {
        ...
        foreach ($files as $key => $file) {
            if ($file[0] === '.' || in_array(substr($file, 0, strrpos($file, '.')), $this->all_iso_lang)) {
                ...
                if ($type_clear === 'file' && !in_array(substr($file, strrpos($file, '.')), $arr_good_ext)) {
                    unset($files[$key]);
                } else {
                    if ($type_clear === 'directory' && (!is_dir($path . $file) || in_array($file, $arr_exclude))) {
                        unset($files[$key]);
                    }
                }
            }
    ...
    public function getFileToParseByTypeTranslation()
    {
        ...
        switch ($this->type_selected) {
            case 'front':
                ...
                break;
            case 'back':
                $directories = array('php' => array(_PS_ADMIN_CONTROLLER_DIR_ . '/' => scandir(_PS_ADMIN_CONTROLLER_DIR_), _PS_OVERRIDE_DIR_ . 'controllers/admin/' => scandir(_PS_OVERRIDE_DIR_ . 'controllers/admin/'), _PS_CLASS_DIR_ . 'helper/' => scandir(_PS_CLASS_DIR_ . 'helper/'), _PS_CLASS_DIR_ . 'controller/' => array('AdminController.php'), _PS_CLASS_DIR_ => array('PaymentModule.php')), 'tpl' => $this->listFiles(_PS_ADMIN_DIR_ . '/themes/'), 'specific' => array(_PS_ADMIN_DIR_ . '/' => array('header.inc.php', 'footer.inc.php', 'index.php', 'functions.php')));
            ...
            case 'pdf':
                ...
                $directories = array('php' => array(_PS_CLASS_DIR_ . 'pdf/' => scandir(_PS_CLASS_DIR_ . 'pdf/'), _PS_OVERRIDE_DIR_ . 'classes/pdf/' => scandir(_PS_OVERRIDE_DIR_ . 'classes/pdf/')), 'tpl' => array(_PS_PDF_DIR_ => scandir(_PS_PDF_DIR_), _PS_THEME_SELECTED_DIR_ . 'pdf/' => $tpl_theme));
                break;
            case 'mails':
                $directories['php'] = array(_PS_FRONT_CONTROLLER_DIR_ => scandir(_PS_FRONT_CONTROLLER_DIR_), _PS_ADMIN_CONTROLLER_DIR_ => scandir(_PS_ADMIN_CONTROLLER_DIR_), _PS_OVERRIDE_DIR_ . 'controllers/front/' => scandir(_PS_OVERRIDE_DIR_ . 'controllers/front/'), _PS_OVERRIDE_DIR_ . 'controllers/admin/' => scandir(_PS_OVERRIDE_DIR_ . 'controllers/admin/'), _PS_ADMIN_DIR_ . '/' => scandir(_PS_ADMIN_DIR_), _PS_ADMIN_DIR_ . '/tabs/' => scandir(_PS_ADMIN_DIR_ . '/tabs'));
                // Get all files for folders classes/ and override/classes/ recursively
                $directories['php'] = array_merge($directories['php'], $this->listFiles(_PS_CLASS_DIR_, array(), 'php'));
                $directories['php'] = array_merge($directories['php'], $this->listFiles(_PS_OVERRIDE_DIR_ . 'classes/', array(), 'php'));
                ...
                if (Tools::isSubmit('submitExport')) {
                    ...
                    if (Tools::isSubmit('submitImport')) {
                        ...
                        if (Tools::isSubmit('submitAddLanguage')) {
                            ...
                            if (Tools::isSubmit('submitTranslationsFront')) {
                                ...
                                if (Tools::isSubmit('submitTranslationsPdf')) {
                                    ...
                                    if (Tools::isSubmit('submitTranslationsBack')) {
                                        if ($this->tabAccess['edit'] === '1') {
                                            $this->writeTranslationFile();
                                        } else {
                                            $this->errors[] = Tools::displayError('You do not have permission to edit this.');
                                        ...
                                        if (Tools::isSubmit('submitTranslationsErrors')) {
                                            if ($this->tabAccess['edit'] === '1') {
                                                $this->writeTranslationFile();
                                            } else {
                                                $this->errors[] = Tools::displayError('You do not have permission to edit this.');
                                            ...
                                            if (Tools::isSubmit('submitTranslationsFields')) {
                                                if ($this->tabAccess['edit'] === '1') {
                                                    $this->writeTranslationFile();
                                                } else {
                                                    $this->errors[] = Tools::displayError('You do not have permission to edit this.');
...
?>')) {
    ...
    public function initFormFront()
    {
        ...
        $this->tpl_view_vars = array_merge($this->tpl_view_vars, array('missing_translations' => $missing_translations_front, 'count' => $count, 'limit_warning' => $this->displayLimitPostWarning($count), 'tabsArray' => $tabs_array));
    ...
    public function initFormBack()
    {
        ...
        $this->tpl_view_vars = array_merge($this->tpl_view_vars, array('count' => $count, 'limit_warning' => $this->displayLimitPostWarning($count), 'tabsArray' => $tabs_array, 'missing_translations' => $missing_translations_back));
        // Add js variables needed for autotranslate
        //$this->tpl_view_vars = array_merge($this->tpl_view_vars, $this->initAutoTranslate());
        $this->initToolbar();
        $this->base_tpl_view = 'translation_form.tpl';
...
?>');
    ...
    protected function recursiveGetModuleFiles($path, &$array_files, $module_name, $lang_file, $is_default = false)
    {
        ...
        if (!empty($files_for_module)) {
            $array_files[] = array('file_name' => $lang_file, 'dir' => $path, 'files' => $files_for_module, 'module' => $module_name, 'is_default' => $is_default, 'theme' => $this->theme_selected);
        }
