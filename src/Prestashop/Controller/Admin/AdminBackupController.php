        ...
        if ($this->display == 'add' && is_writable(_PS_ADMIN_DIR_ . '/backups/')) {
            if ($object = $this->loadObject()) {
                if (!$object->add()) {
                    ...
                    $this->context->smarty->assign(array('conf' => $this->l('It appears the backup was successful, however you must download and carefully verify the backup file before proceeding. '), 'backup_url' => $object->getBackupURL(), 'backup_weight' => number_format(filesize($object->id) * 1.0E-6, 2, '.', '')));
                }
            }
        }
        parent::postProcess();
