...
use Prestashop\Controller\Admin\AdminCmsCategoriesController;
use Prestashop\Controller\Admin\AdminCmsController;
use Prestashop\CMS;
/*
...
*/
class AdminCmsContentController extends AdminController
{
    ...
    public function initContent()
    {
        ...
        if ($this->display == 'edit_category') {
            ...
            if ($this->display == 'edit_page') {
                $this->content .= $this->admin_cms->renderForm();
            } else {
                if ($this->display == 'view_page') {
                    $fixme = 'fixme';
                } else {
                    ...
                    $this->context->smarty->assign(array('cms_breadcrumb' => getPath($cat_bar_index, $id_cms_category, '', '', 'cms')));
                }
            }
        }
    ...
    {
        if (Tools::isSubmit('submitDelcms') || Tools::isSubmit('previewSubmitAddcmsAndPreview') || Tools::isSubmit('submitAddcms') || Tools::isSubmit('submitBulkdeletecms') || Tools::isSubmit('deletecms') || Tools::isSubmit('viewcms') || Tools::isSubmit('statuscms') && Tools::isSubmit('id_cms') || Tools::isSubmit('way') && Tools::isSubmit('id_cms') && Tools::isSubmit('position')) {
            $this->admin_cms->postProcess();
        } elseif (Tools::isSubmit('submitDelcms_category') || Tools::isSubmit('submitAddcms_categoryAndBackToParent') || Tools::isSubmit('submitBulkdeletecms_category') || Tools::isSubmit('submitAddcms_category') || Tools::isSubmit('deletecms_category') || Tools::isSubmit('statuscms_category') && Tools::isSubmit('id_cms_category') || Tools::isSubmit('position') && Tools::isSubmit('id_cms_category_to_move')) {
        ...
        if ((Tools::isSubmit('submitAddcms_category') || Tools::isSubmit('submitAddcms_categoryAndStay')) && count($this->admin_cms_categories->errors) || Tools::isSubmit('updatecms_category') || Tools::isSubmit('addcms_category')) {
            ...
            if ((Tools::isSubmit('submitAddcms') || Tools::isSubmit('submitAddcmsAndStay')) && count($this->admin_cms->errors) || Tools::isSubmit('updatecms') || Tools::isSubmit('addcms')) {
