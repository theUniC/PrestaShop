...
class AdminTaxRulesGroupController extends AdminController
{
    ...
    public function __construct()
    {
        ...
        $this->fields_list = array('id_tax_rules_group' => array('title' => $this->l('ID'), 'width' => 25), 'name' => array('title' => $this->l('Name'), 'width' => 'auto'), 'active' => array('title' => $this->l('Enabled'), 'width' => 25, 'active' => 'status', 'type' => 'bool', 'orderby' => false, 'align' => 'center'));
    ...
    public function initRulesList($id_group)
    {
        ...
        $this->fields_list = array('country_name' => array('title' => $this->l('Country'), 'width' => 140), 'state_name' => array('title' => $this->l('State'), 'width' => 140), 'zipcode' => array('title' => $this->l('Zip Code'), 'width' => 25), 'behavior' => array('title' => $this->l('Behavior'), 'width' => 25), 'rate' => array('title' => $this->l('Tax'), 'width' => 25), 'description' => array('title' => $this->l('Description'), 'width' => 25));
        $this->addRowAction('edit');
        $this->addRowAction('delete');
        $this->_select = '
    ...
    public function renderForm()
    {
        ...
        if (!isset($obj->id)) {
            ...
            $this->no_back = true;
            $this->toolbar_btn['new'] = array('href' => '#', 'desc' => $this->l('Add a new tax rule'));
            $content = parent::renderForm();
                        ...
                        foreach (array($tr->zipcode_from, $tr->zipcode_to) as $zip_code) {
                            if ($zip_code) {
                                if (!$country->checkZipCode($zip_code)) {
                                    $this->errors[] = sprintf(Tools::displayError('Zip/Postal code is invalid. Must be typed as follows: %s for %s'), str_replace('C', $country->iso_code, str_replace('N', '0', str_replace('L', 'A', $country->zip_code_format))), $country->name);
                                }
                            }
                        }
                    }
