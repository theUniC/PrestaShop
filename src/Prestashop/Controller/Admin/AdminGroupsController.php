...
class AdminGroupsController extends AdminController
{
    ...
    protected function formatCategoryDiscountList($id_group)
    {
        ...
        foreach ($group_reductions as $category) {
            ...
            $category_reductions[(int) $category['id_category']] = array('path' => getPath(self::$currentIndex . '?tab=AdminCategories', (int) $category['id_category']), 'reduction' => (double) $category['reduction'] * 100, 'id_category' => (int) $category['id_category']);
        }
        if (is_array($category_reduction)) {
            foreach ($category_reduction as $key => $val) {
                if (!array_key_exists($key, $category_reductions)) {
                    $category_reductions[(int) $key] = array('path' => getPath(self::$currentIndex . '?tab=AdminCategories', $key), 'reduction' => (double) $val * 100, 'id_category' => (int) $key);
                }
            }
        }
    ...
    public function renderList()
    {
        ...
        $guest_group_information = sprintf($this->l('%s - Customer who placed an order through Guest Checkout.'), '<b>' . $guest->name[$this->context->language->id] . '</b>');
