...
*/
class AdminShopUrlController extends AdminController
{
    ...
    public function initToolbar()
    {
        ...
        if (!$this->display && $this->id_shop) {
            $this->toolbar_btn['edit'] = array('desc' => $this->l('Edit this shop'), 'href' => $this->context->link->getAdminLink('AdminShop') . '&amp;updateshop&amp;id_shop=' . $this->id_shop);
        }
        parent::initToolbar();
        $this->show_toolbar = false;
        if (isset($this->toolbar_btn['new'])) {
            $this->toolbar_btn['new'] = array('desc' => $this->l('Add a new URL'), 'href' => $this->context->link->getAdminLink('AdminShopUrl') . '&amp;add' . $this->table . '&amp;id_shop=' . $this->id_shop);
        }
    ...
    public function initContent()
    {
        ...
        $this->context->smarty->assign(array('toolbar_scroll' => 1, 'toolbar_btn' => $this->toolbar_btn, 'title' => $this->toolbar_title, 'selected_tree_id' => $this->display == 'edit' ? 'tree-url-' . $this->id_object : (Tools::getValue('id_shop') ? 'tree-shop-' . Tools::getValue('id_shop') : '')));
    }
    ...
    public function displayDeleteLink($token = null, $id, $name = null)
    {
        ...
        $data = array($this->identifier => $id, 'href' => Tools::safeOutput(self::$currentIndex . '&' . $this->identifier . '=' . $id . '&delete' . $this->table . '&id_shop=' . $this->id_shop . '&token=' . ($token != null ? $token : $this->token)), 'action' => self::$cache_lang['Delete']);
