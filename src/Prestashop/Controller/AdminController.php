...
class AdminController extends Controller
{
    ...
    protected function initTabModuleList()
    {
        ...
        if (is_array($this->tab_modules_list['default_list']) && count($this->tab_modules_list['default_list'])) {
            ...
            $this->addToolBarModulesListButton();
            $this->context->smarty->assign(array('tab_modules_list' => implode(',', $this->tab_modules_list['slider_list']), 'admin_module_ajax_url' => $this->context->link->getAdminLink('AdminModules'), 'back_tab_modules_list' => $this->context->link->getAdminLink(Tools::getValue('controller')), 'tab_modules_open' => (int) Tools::getValue('tab_modules_open')));
        }
    }
    protected function addToolBarModulesListButton()
    {
        if (is_array($this->tab_modules_list['slider_list']) && count($this->tab_modules_list['slider_list'])) {
            $this->toolbar_btn['modules-list'] = array('href' => '#', 'desc' => $this->l('Modules List'));
        }
    }
    /**
    ...
    public function initFooter()
    {
        ...
        $this->context->smarty->assign(array('ps_version' => _PS_VERSION_, 'timer_start' => $this->timer_start, 'iso_is_fr' => strtoupper($this->context->language->iso_code) == 'FR'));
    }
    ...
    public function setMedia()
    {
        ...
        $this->addJS(array(_PS_JS_DIR_ . 'admin.js', _PS_JS_DIR_ . 'toggle.js', _PS_JS_DIR_ . 'tools.js', _PS_JS_DIR_ . 'ajax.js', _PS_JS_DIR_ . 'toolbar.js'));
        if (!Tools::getValue('submitFormAjax')) {
    ...
    public function init()
    {
        ...
        $this->context->smarty->assign(array('table' => $this->table, 'current' => self::$currentIndex, 'token' => $this->token, 'stock_management' => (int) Configuration::get('PS_STOCK_MANAGEMENT')));
    ...
    public function validateRules($class_name = false)
    {
            ...
            foreach ($rules['required'] as $field) {
                if (($value = Tools::getValue($field)) == false && (string) $value != '0') {
                    if (!Tools::getValue($this->identifier) || $field != 'passwd' && $field != 'no-picture') {
                        $this->errors[] = sprintf(Tools::displayError('The %s field is required.'), call_user_func(array($class_name, 'displayFieldName'), $field, $class_name));
                    }
                }
            }
        }
        ...
        if (isset($rules['requiredLang']) && is_array($rules['requiredLang'])) {
            foreach ($rules['requiredLang'] as $field_lang) {
                if (($empty = Tools::getValue($field_lang . '_' . $default_language->id)) === false || $empty !== '0' && empty($empty)) {
                    $this->errors[] = sprintf(Tools::displayError('The field %1$s is required at least in %2$s.'), call_user_func(array($class_name, 'displayFieldName'), $field_lang, $class_name), $default_language->name);
                }
            }
        }
        ...
        if (isset($rules['size']) && is_array($rules['size'])) {
            foreach ($rules['size'] as $field => $max_length) {
                if (Tools::getValue($field) !== false && Tools::strlen(Tools::getValue($field)) > $max_length) {
                    $this->errors[] = sprintf(Tools::displayError('The %1$s field is too long (%2$d chars max).'), call_user_func(array($class_name, 'displayFieldName'), $field, $class_name), $max_length);
                }
            }
        }
            ...
            foreach ($rules['sizeLang'] as $field_lang => $max_length) {
                foreach ($languages as $language) {
                    ...
                    if ($field_lang_value !== false && Tools::strlen($field_lang_value) > $max_length) {
                        $this->errors[] = sprintf(Tools::displayError('The field %1$s (%2$s) is too long (%3$d chars max, html chars including).'), call_user_func(array($class_name, 'displayFieldName'), $field_lang, $class_name), $language['name'], $max_length);
                    }
                }
            }
        }
            ...
            foreach ($rules['validate'] as $field => $function) {
                if (($value = Tools::getValue($field)) !== false && $field != 'passwd') {
                    if (!Validate::$function($value) && !empty($value)) {
                        $this->errors[] = sprintf(Tools::displayError('The %s field is invalid.'), call_user_func(array($class_name, 'displayFieldName'), $field, $class_name));
                    }
                }
            }
        }
        ...
        if (($value = Tools::getValue('passwd')) != false) {
            if ($class_name == 'Employee' && !Validate::isPasswdAdmin($value)) {
                $this->errors[] = sprintf(Tools::displayError('The %s field is invalid.'), call_user_func(array($class_name, 'displayFieldName'), 'passwd', $class_name));
                ...
                foreach ($languages as $language) {
                    if (($value = Tools::getValue($field_lang . '_' . $language['id_lang'])) !== false && !empty($value)) {
                        if (!Validate::$function($value)) {
                            $this->errors[] = sprintf(Tools::displayError('The %1$s field (%2$s) is invalid.'), call_user_func(array($class_name, 'displayFieldName'), $field_lang, $class_name), $language['name']);
                        }
                    }
                }
            }
    ...
    public function fillModuleData(&$module, $output_type = 'link', $back = null)
    {
        ...
        $module->options['uninstall_onclick'] = !$module->onclick_option ? empty($module->confirmUninstall) ? '' : 'return confirm(\'' . addslashes($module->confirmUninstall) . '\');' : $obj->onclickOption('uninstall', $module->options['uninstall_url']);
