...
*/
class OrderOpcController extends ParentOrderController
{
                            ...
                            if ($this->context->customer->isLogged(true)) {
                                ...
                                if ($address_delivery->id_customer != $this->context->customer->id || $address_invoice->id_customer != $this->context->customer->id) {
                                    ...
                                    if (!count($this->errors)) {
                                        ...
                                        $wrapping_fees = $this->context->cart->getGiftWrappingPrice(false);
                                        $wrapping_fees_tax_inc = $wrapping_fees = $this->context->cart->getGiftWrappingPrice();
                                        $result = array_merge($result, array('HOOK_TOP_PAYMENT' => Hook::exec('displayPaymentTop'), 'HOOK_PAYMENT' => $this->_getPaymentMethods(), 'gift_price' => Tools::displayPrice(Tools::convertPrice(Product::getTaxCalculationMethod() == 1 ? $wrapping_fees : $wrapping_fees_tax_inc, new Currency((int) $this->context->cookie->id_currency))), 'carrier_data' => $this->_getCarrierList()), $this->getFormatedSummaryDetail());
    ...
    {
        $this->context->smarty->assign(array('HOOK_TOP_PAYMENT' => $this->isLogged ? Hook::exec('displayPaymentTop') : '', 'HOOK_PAYMENT' => $this->_getPaymentMethods()));
    }
