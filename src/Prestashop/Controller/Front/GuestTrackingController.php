...
*/
class GuestTrackingController extends FrontController
{
    public $ssl = true;
                    ...
                    if (!Validate::isEmail($email)) {
                        ...
                        if (!Customer::customerExists($email, false, false)) {
                            ...
                            if (Customer::customerExists($email, false, true)) {
                                $this->errors[] = Tools::displayError('Your guest account has already been transformed into a customer account.') . ' ' . Tools::displayError('Please login to your customer account to view this order. This section is reserved for guest accounts.');
                                ...
                                if (!count($order_collection)) {
                                    ...
                                    if (!$order_collection->getFirst()->isAssociatedAtGuest($email)) {
                                        $this->errors[] = Tools::displayError('Invalid order reference');
                                    } else {
