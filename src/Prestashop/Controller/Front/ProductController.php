...
class ProductController extends FrontController
{
    ...
    public function setMedia()
    {
        ...
        if ($this->context->getMobileDevice() == false) {
            ...
            $this->addJS(array(_THEME_JS_DIR_ . 'tools.js', _THEME_JS_DIR_ . 'product.js'));
        } else {
            $this->addJqueryPlugin(array('scrollTo', 'serialScroll'));
            $this->addJS(array(_THEME_JS_DIR_ . 'tools.js', _THEME_MOBILE_JS_DIR_ . 'product.js', _THEME_MOBILE_JS_DIR_ . 'jquery.touch-gallery.js'));
        }
    ...
    public function init()
    {
        ...
        if (!Validate::isLoadedObject($this->product)) {
            ...
            if (!$this->product->isAssociatedToShop() || !$this->product->active) {
                ...
                // Load category
                if (isset($_SERVER['HTTP_REFERER']) && strstr($_SERVER['HTTP_REFERER'], Tools::getHttpHost()) && stripos($_SERVER['HTTP_REFERER'], '.html') === false && preg_match('~^(.*)\\/([0-9]+)\\-((?![.]+html).*)|(.*)id_category=([0-9]+)(.*)$~', $_SERVER['HTTP_REFERER'], $regs)) {
...
', $text_field['value']);
    ...
    protected function assignCategory()
    {
        ...
        if ($this->category !== false && Validate::isLoadedObject($this->category) && $this->category->inShop() && $this->category->isAssociatedToShop()) {
            ...
            $this->context->smarty->assign(array('category' => $this->category, 'subCategories' => $this->category->getSubCategories($this->context->language->id, true), 'id_category_current' => (int) $this->category->id, 'id_category_parent' => (int) $this->category->id_parent, 'return_category_name' => Tools::safeOutput($this->category->name)));
        ...
        foreach ($_FILES as $field_name => $file) {
            if (in_array($field_name, $authorized_file_fields) && isset($file['tmp_name']) && !empty($file['tmp_name'])) {
                ...
                if (!ImageManager::resize($tmp_name, _PS_UPLOAD_DIR_ . $file_name)) {
                    ...
                    $this->errors[] = Tools::displayError('An error occurred during the image upload process.');
                } elseif (!chmod(_PS_UPLOAD_DIR_ . $file_name, 511) || !chmod(_PS_UPLOAD_DIR_ . $file_name . '_small', 511)) {
                    $this->errors[] = Tools::displayError('An error occurred during the image upload process.');
                } else {
    ...
    protected function formatQuantityDiscounts($specific_prices, $price, $tax_rate, $ecotax_amount)
    {
        foreach ($specific_prices as $key => &$row) {
            ...
            if ($row['price'] >= 0) {
                $cur_price = (Product::$_taxCalculationMethod == PS_TAX_EXC ? $row['price'] : $row['price'] * (1 + $tax_rate / 100)) + (double) $ecotax_amount;
