...
class Image extends ObjectModel
{
    ...
    protected static $access_rights = 509;
    /**
     * @see ObjectModel::$definition
     */
    public static $definition = array('table' => 'image', 'primary' => 'id_image', 'multilang' => true, 'fields' => array('id_product' => array('type' => self::TYPE_INT, 'validate' => 'isUnsignedId', 'required' => true), 'position' => array('type' => self::TYPE_INT, 'validate' => 'isUnsignedInt'), 'cover' => array('type' => self::TYPE_BOOL, 'validate' => 'isBool', 'shop' => true)));
    ...
    public static function deleteAllImages($path, $format = 'jpg')
    {
        ...
        foreach (scandir($path) as $file) {
            if (preg_match('/^[0-9]+(\\-(.*))?\\.' . $format . '$/', $file)) {
                ...
                if (is_dir($path . $file) && preg_match('/^[0-9]$/', $file)) {
