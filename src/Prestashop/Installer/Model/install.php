...
namespace Prestashop\Installer\Model;
...
use Prestashop\Installer\Model;
use Prestashop\Installer\Session;
use Prestashop\Installer\SQLLoader;
use Prestashop\Installer\Exception;
use Prestashop\Installer\XMLLoader;
use Prestashop\LocalizationPack;
use Prestashop\Log\FileLogger;
...
';
    ...
    public function installDatabase($clear_database = false)
    {
        ...
        $sql_loader->setMetaData(array('PREFIX_' => _DB_PREFIX_, 'ENGINE_TYPE' => _MYSQL_ENGINE_));
        try {
    ...
    public function installLanguages($languages_list = null)
    {
        ...
        foreach ($languages_list as $iso) {
            if (!file_exists(_PS_INSTALL_LANGS_PATH_ . $iso . '/language.xml')) {
                throw new Exception($this->language->l('File "language.xml" not found for language iso "%s"', $iso));
            }
            if (!($xml = @simplexml_load_file(_PS_INSTALL_LANGS_PATH_ . $iso . '/language.xml'))) {
                throw new Exception($this->language->l('File "language.xml" not valid for language iso "%s"', $iso));
            }
            $params_lang = array('name' => (string) $xml->name, 'iso_code' => substr((string) $xml->language_code, 0, 2));
            if (Session::getInstance()->safe_mode) {
            ...
            if (!($id_lang = Language::getIdByIso($iso))) {
                throw new Exception($this->language->l('Cannot install language "%s"', $xml->name ? $xml->name : $iso));
            }
            ...
            if (is_writable(_PS_IMG_DIR_ . 'l/')) {
                if (!copy(_PS_INSTALL_LANGS_PATH_ . $iso . '/flag.jpg', _PS_IMG_DIR_ . 'l/' . $id_lang . '.jpg')) {
                    throw new Exception($this->language->l('Cannot copy flag language "%s"', _PS_INSTALL_LANGS_PATH_ . $iso . '/flag.jpg => ' . _PS_IMG_DIR_ . 'l/' . $id_lang . '.jpg'));
                }
            }
        }
        return $languages;
    ...
    public function copyLanguageImages($iso)
    {
        ...
        $list = array('products' => _PS_PROD_IMG_DIR_, 'categories', _PS_CAT_IMG_DIR_, 'manufacturers' => _PS_MANU_IMG_DIR_, 'suppliers' => _PS_SUPP_IMG_DIR_, 'scenes' => _PS_SCENE_IMG_DIR_, 'stores' => _PS_STORE_IMG_DIR_, null => _PS_IMG_DIR_ . 'l/');
    ...
    public function configureShop(array $data = array())
    {
        ...
        if (isset($data['admin_email'])) {
            ...
            $contacts = new Collection('\Prestashop\Contact');
    ...
    public function installModulesAddons($module = null)
    {
        ...
        if (!Session::getInstance()->safe_mode) {
            foreach ($addons_modules as $addons_module) {
                if (!is_dir(_PS_MODULE_DIR_ . $addons_module['name']) && file_put_contents(_PS_MODULE_DIR_ . $addons_module['name'] . '.zip', Tools::addonsRequest('module', array('id_module' => $addons_module['id_module'])))) {
