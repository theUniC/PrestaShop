...
class XMLLoader
{
	...
	{
		$this->language = Languages::getInstance();
		$this->setDefaultPath();
	}
	...
	public function populateEntity($entity)
	{
		...
		if (!is_object($xml) || !$xml->fields)
			throw new Exception('List of fields not found for entity '.$entity);
	...
	protected function loadEntity($entity, $iso = null)
	{
		if (!isset($this->cache_xml_entity[$this->path_type][$entity][$iso]))
		{
			...
			if (!file_exists($path))
				throw new Exception('XML data file '.$entity.'.xml not found');

			$this->cache_xml_entity[$this->path_type][$entity][$iso] = @simplexml_load_file($path, '\Prestashop\Installer\SimplexmlElement');
			if (!$this->cache_xml_entity[$this->path_type][$entity][$iso])
				throw new Exception('XML data file '.$entity.'.xml invalid');
		}
	...
	public function createEntity($entity, $identifier, $classname, array $data, array $data_lang = array())
	{
		...
		if ($classname)
		{
            ...
            $classname = '\Prestashop\\' . $classname;
