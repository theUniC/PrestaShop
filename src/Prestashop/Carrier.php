...
use Prestashop\Cart;
use Prestashop\Range\RangePrice;
use Prestashop\Range\RangeWeight;
...
class Carrier extends ObjectModel
{
    ...
    protected $webserviceParameters = array('fields' => array('deleted' => array(), 'is_module' => array(), 'id_tax_rules_group' => array('getter' => 'getIdTaxRulesGroup', 'setter' => 'setTaxRulesGroup', 'xlink_resource' => array('resourceName' => 'tax_rules_group'))));
        ...
        if (!isset(self::$price_by_weight[$cache_key])) {
            $sql = 'SELECT d.`price`
					...
					WHERE d.`id_zone` = ' . (int) $id_zone . '
						AND ' . (double) $total_weight . ' >= w.`delimiter1`
						AND ' . (double) $total_weight . ' < w.`delimiter2`
    ...
    public static function getCarriersForOrder($id_zone, $groups = null, $cart = null)
    {
        ...
        foreach ($result as $k => $row) {
            ...
            if ($shipping_method != Carrier::SHIPPING_METHOD_FREE) {
                ...
                if ($shipping_method == Carrier::SHIPPING_METHOD_WEIGHT && $carrier->getMaxDeliveryPriceByWeight($id_zone) === false || $shipping_method == Carrier::SHIPPING_METHOD_PRICE && $carrier->getMaxDeliveryPriceByPrice($id_zone) === false) {
                    unset($result[$k]);
                    continue;
                }
                ...
                if ($row['range_behavior']) {
                    ...
                    if ($shipping_method == Carrier::SHIPPING_METHOD_WEIGHT && !Carrier::checkDeliveryPriceByWeight($row['id_carrier'], $cart->getTotalWeight(), $id_zone) || $shipping_method == Carrier::SHIPPING_METHOD_PRICE && !Carrier::checkDeliveryPriceByPrice($row['id_carrier'], $cart->getOrderTotal(true, Cart::BOTH_WITHOUT_SHIPPING), $id_zone, $id_currency)) {
                        unset($result[$k]);
                        continue;
                    }
                }
            ...
            if (count($res)) {
                foreach ($res as $val) {
                    Db::getInstance()->execute('
						...
						VALUES (' . $this->id . ',' . (double) $val['delimiter1'] . ',' . (double) $val['delimiter2'] . ')');
        ...
        if ($carrier_list) {
            ...
            foreach ($carrier_list as $carrier) {
                foreach ($id_group_list as $id_group) {
                    $data[] = array('id_carrier' => $carrier['id_carrier'], 'id_group' => $id_group);
                }
            }
