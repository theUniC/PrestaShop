...
use Prestashop\Order\OrderInvoice;
...
class OrderDetail extends ObjectModel
{
    ...
    public function saveTaxCalculator(Order $order, $replace = false)
    {
        ...
        foreach ($this->tax_calculator->getTaxesAmount($discounted_price_tax_excl) as $id_tax => $amount) {
            $unit_amount = (double) Tools::ps_round($amount, 2);
            $total_amount = $unit_amount * $this->product_quantity;
            $values .= '(' . (int) $this->id . ',' . (double) $id_tax . ',' . $unit_amount . ',' . (double) $total_amount . '),';
        }
        if ($replace) {
    ...
    protected function setDetailProductPrice(Order $order, Cart $cart, $product)
    {
        ...
        if ($product['id_supplier'] > 0) {
            $this->purchase_supplier_price = (double) ProductSupplier::getProductPrice((int) $product['id_supplier'], $product['id_product'], $product['id_product_attribute'], true);
        }
        $this->setSpecificPrice($order, $product);
        $this->group_reduction = (double) Group::getReduction((int) $order->id_customer);
        $shop_id = $this->context->shop->id;
        $quantityDiscount = SpecificPrice::getQuantityDiscount((int) $product['id_product'], $shop_id, (int) $cart->id_currency, (int) $this->vat_address->id_country, (int) $this->customer->id_default_group, (int) $product['cart_quantity'], false, null, null, $null, true, true, $this->context);
        $unitPrice = Product::getPriceStatic((int) $product['id_product'], true, $product['id_product_attribute'] ? intval($product['id_product_attribute']) : null, 2, null, false, true, 1, false, (int) $order->id_customer, null, (int) $order->{Configuration::get('PS_TAX_ADDRESS_TYPE')}, $null, true, true, $this->context);
    ...
    protected function create(Order $order, Cart $cart, $product, $id_order_state, $id_order_invoice, $use_taxes = true, $id_warehouse = 0)
    {
        ...
        $this->product_weight = (double) ($product['id_product_attribute'] ? $product['weight_attribute'] : $product['weight']);
        $this->id_warehouse = $id_warehouse;
        $productQuantity = (int) Product::getQuantity($this->product_id, $this->product_attribute_id);
        $this->product_quantity_in_stock = $productQuantity - (int) $product['cart_quantity'] < 0 ? $productQuantity : (int) $product['cart_quantity'];
