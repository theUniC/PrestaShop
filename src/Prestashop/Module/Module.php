...
abstract class Module
{
        ...
        if ($upgrade_detail['available_upgrade']) {
            if ($upgrade_detail['success']) {
                ...
                if (!$upgrade_detail['number_upgraded']) {
                    ...
                    $this->_errors[] = $this->l('Upgraded from: ') . $upgrade_detail['upgraded_from'] . $this->l(' to ') . $upgrade_detail['upgraded_to'];
    ...
    public static function initUpgradeModule($module)
    {
        ...
        self::$modules_cache[$module->name]['upgrade'] = array('success' => false, 'available_upgrade' => 0, 'number_upgraded' => 0, 'number_upgrade_left' => 0, 'upgrade_file_left' => array(), 'version_fail' => 0, 'upgraded_from' => 0, 'upgraded_to' => 0);
            ...
            foreach ($files as $file) {
                if (!in_array($file, array('.', '..', '.svn', 'index.php'))) {
                    ...
                    if (count($tab) == 2 && (Tools::version_compare($file_version, $module_version, '<=') && Tools::version_compare($file_version, $registered_version, '>'))) {
                        $list[] = array('file' => $upgrade_path . $file, 'version' => $file_version, 'upgrade_function' => 'upgrade_module_' . str_replace('.', '_', $file_version));
                    }
                }
            }
        }
    ...
    public function enable($forceAll = false)
    {
        ...
        $sql = 'SELECT `id_shop` FROM `' . _DB_PREFIX_ . 'module_shop`
				WHERE `id_module` = ' . $this->id . (!$forceAll ? ' AND `id_shop` IN(' . implode(', ', $list) . ')' : '');
        ...
        foreach ($list as $id) {
            if (!in_array($id, $items)) {
                Db::getInstance()->insert('module_shop', array('id_module' => $this->id, 'id_shop' => $id));
            }
        }
        return true;
    }
    ...
    public function registerExceptions($id_hook, $excepts, $shop_list = null)
    {
        ...
        foreach ($shop_list as $shop_id) {
            foreach ($excepts as $except) {
                ...
                $insertException = array('id_module' => (int) $this->id, 'id_hook' => (int) $id_hook, 'id_shop' => (int) $shop_id, 'file_name' => pSQL($except));
