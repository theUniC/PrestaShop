...
class Product extends ObjectModel
{
    ...
    public static function cleanPositions($id_category)
    {
        ...
        $total = count($result);
        for ($i = 0; $i < $total; $i++) {
            $return &= Db::getInstance()->update('category_product', array('position' => $i), '`id_category` = ' . (int) $id_category . ' AND `id_product` = ' . (int) $result[$i]['id_product']);
        }
    ...
    {
        Db::getInstance()->update('product', array('is_virtual' => (int) $id_product), 'id_product = ' . (int) $id_product);
    }
    /**
    ...
    public function addToCategories($categories = array())
    {
        ...
        foreach ($categories as $new_id_categ) {
            if (!in_array($new_id_categ, $current_categories)) {
                $product_cats[] = array('id_category' => (int) $new_id_categ, 'id_product' => (int) $this->id, 'position' => (int) $new_categ_pos[$new_id_categ]);
            }
        }
        ...
        foreach ($combinations as $key => $combination) {
            ...
            if (!$id_combination) {
                ...
                foreach ($attributes[$key] as $id_attribute) {
                    $attribute_list[] = array('id_product_attribute' => (int) $obj->id, 'id_attribute' => (int) $id_attribute);
                }
    ...
    {
        return ObjectModel::updateMultishopTable('Combination', array('default_on' => 0), 'id_product = ' . (int) $this->id);
    }
    public function setDefaultAttribute($id_product_attribute)
    {
        $result = ObjectModel::updateMultishopTable('Combination', array('default_on' => 1), '`id_product` = ' . (int) $this->id . ' AND a.`id_product_attribute` = ' . (int) $id_product_attribute);
        $result &= ObjectModel::updateMultishopTable('product', array('cache_default_attribute' => (int) $id_product_attribute), 'a.`id_product` = ' . (int) $this->id);
    ...
    public static function updateDefaultAttribute($id_product)
    {
        ...
        $result = Db::getInstance()->update('product_shop', array('cache_default_attribute' => $id_default_attribute), 'id_product = ' . (int) $id_product);
        $result &= Db::getInstance()->update('product', array('cache_default_attribute' => $id_default_attribute), 'id_product = ' . (int) $id_product);
    ...
    public static function updateCacheAttachment($id_product)
    {
        ...
        return Db::getInstance()->update('product', array('cache_has_attachments' => (int) $value), 'id_product = ' . (int) $id_product);
    }
    /**
    ...
    public function deleteCustomization()
    {
        return Db::getInstance()->execute('DELETE FROM `' . _DB_PREFIX_ . 'customization_field`
                WHERE `id_product` = ' . (int) $this->id) && Db::getInstance()->execute('DELETE FROM `' . _DB_PREFIX_ . 'customization_field_lang`
    ...
    {
        return Db::getInstance()->execute('DELETE FROM `' . _DB_PREFIX_ . 'search_index`
                WHERE `id_product` = ' . (int) $this->id) && Db::getInstance()->execute('DELETE FROM `' . _DB_PREFIX_ . 'search_word`
    ...
    public function setCarriers($carrier_list)
    {
        ...
        foreach ($carrier_list as $carrier) {
            $data[] = array('id_product' => (int) $this->id, 'id_carrier_reference' => (int) $carrier, 'id_shop' => (int) $this->id_shop);
        }
    ...
    public static function priceCalculation($id_shop, $id_product, $id_product_attribute, $id_country, $id_state, $zipcode, $id_currency, $id_group, $quantity, $use_tax, $decimals, $only_reduc, $use_reduc, $with_ecotax, &$specific_price, $use_group_reduction, $id_customer = 0, $use_customer_price = true, $id_cart = 0, $real_quantity = 0)
    {
        ...
        if ($use_group_reduction) {
            ...
            if (!empty($reduction_from_category) && (double) $reduction_from_category == 0) {
    ...
    public function changeAccessories($accessories_id)
    {
        foreach ($accessories_id as $id_product_2) {
            Db::getInstance()->insert('accessory', array('id_product_1' => (int) $this->id, 'id_product_2' => (int) $id_product_2));
        }
    }
    /**
    ...
    public static function duplicateTags($id_product_old, $id_product_new)
    {
        ...
        $data = array();
        foreach ($tags as $tag) {
            $data[] = array('id_product' => (int) $id_product_new, 'id_tag' => (int) $tag['id_tag']);
        }
    ...
    */
    ...
    {
        $type_information = array(Product::PTYPE_SIMPLE => 'simple', Product::PTYPE_PACK => 'pack', Product::PTYPE_VIRTUAL => 'virtual');
