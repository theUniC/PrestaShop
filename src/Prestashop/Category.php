...
class Category extends ObjectModel
{
    ...
    public function updatePosition($way, $position)
    {
        ...
        $result = Db::getInstance()->execute('
			...
			WHERE cs.`position`
			' . ($way ? '> ' . (int) $moved_category['position'] . ' AND cs.`position` <= ' . (int) $position : '< ' . (int) $moved_category['position'] . ' AND cs.`position` >= ' . (int) $position) . '
        ...
        if (!$id_shop) {
            foreach (Shop::getShops(false) as $shop) {
                if (!$this->existsInShop($shop['id_shop'])) {
                    $data[] = array('id_category' => (int) $this->id, 'id_shop' => (int) $shop['id_shop']);
                }
            }
        } else {
            if (!$this->existsInShop($id_shop)) {
                $data[] = array('id_category' => (int) $this->id, 'id_shop' => (int) $id_shop);
            }
        }
    ...
    {
        return Db::getInstance()->execute('DELETE FROM `' . _DB_PREFIX_ . 'category_shop`
