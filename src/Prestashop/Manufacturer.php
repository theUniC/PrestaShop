...
class Manufacturer extends ObjectModel
{
    ...
    public static function getManufacturers($get_nb_products = false, $id_lang = 0, $active = true, $p = false, $n = false, $all_group = false)
    {
        ...
        $sql .= '
			...
			ORDER BY m.`name` ASC' . ($p ? ' LIMIT ' . ((int) $p - 1) * (int) $n . ',' . (int) $n : '');
        ...
        if ($get_nb_products) {
            ...
            foreach ($manufacturers as $key => $manufacturer) {
                $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('SELECT p.`id_product`
					...
					WHERE m.`id_manufacturer` = ' . (int) $manufacturer['id_manufacturer'] . ($active ? ' AND product_shop.`active` = 1 ' : '') . ($all_group ? '' : ' AND p.`id_product` IN (
