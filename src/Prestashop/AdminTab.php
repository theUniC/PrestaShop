...
abstract class AdminTab
{
    ...
    public static $tabParenting = array('AdminCms' => 'AdminCmsContent', 'AdminCmsCategories' => 'AdminCmsContent', 'AdminOrdersStates' => 'AdminStatuses', 'AdminAttributeGenerator' => 'AdminProducts', 'AdminAttributes' => 'AdminAttributesGroups', 'AdminFeaturesValues' => 'AdminFeatures', 'AdminReturnStates' => 'AdminStatuses', 'AdminStatsTab' => 'AdminStats');
    ...
    public function getList($id_lang, $orderBy = NULL, $orderWay = NULL, $start = 0, $limit = NULL, $id_lang_shop = false)
    {
        ...
        if (!Validate::isOrderBy($orderBy) || !Validate::isOrderWay($orderWay) or !is_numeric($start) || !is_numeric($limit) or !Validate::isUnsignedId($id_lang)) {
        ...
        if (isset($filterKey)) {
            if (!$this->_group) {
                ...
                if (!preg_match('#(\\s|,)\\s*a\\.`?' . pSQL($this->identifier) . '`?(\\s|,|$)#', $this->_group)) {
            ...
            foreach ($categoryData['fields'] as $key => $field) {
                ...
                if (Shop::isFeatureActive()) {
                    if (isset($field['visibility']) && $field['visibility'] > Shop::getContext()) {
                        ...
                        if (Shop::getContext() != Shop::CONTEXT_ALL && !Configuration::isOverridenByCurrentContext($key)) {
                            $isDisabled = true;
                        }
                    }
                }
