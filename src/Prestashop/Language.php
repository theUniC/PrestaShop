...
use \Archive_Tar;
use Prestashop\Controller\Admin\AdminTranslationsController;
/*
...
?>');
            ...
            foreach ($langTables as $name) {
                ...
                foreach ($columns as $column) {
                    if ($identifier != $column['Field'] && $column['Field'] != 'id_lang') {
                        $sql .= '(SELECT `' . $column['Field'] . '`	FROM `' . $name . '` tl	WHERE tl.`id_lang` = ' . (int) $id_lang_default . ($shop_field_exists ? ' AND tl.`id_shop` = ' . (int) $shop->id : '') . ' AND tl.`' . $identifier . '` = `' . str_replace('_lang', '', $name) . '`.`' . $identifier . '`), ';
                    } else {
                        $sql .= '`' . $column['Field'] . '`, ';
                    }
                }
    ...
    public function delete()
    {
        ...
        if (!$this->hasMultishopEntries() || Shop::getContext() == Shop::CONTEXT_ALL) {
            // delete images
            $files_copy = array('/en.jpg', '/en-default-' . ImageType::getFormatedName('thickbox') . '.jpg', '/en-default-' . ImageType::getFormatedName('home') . '.jpg', '/en-default-' . ImageType::getFormatedName('large') . '.jpg', '/en-default-' . ImageType::getFormatedName('medium') . '.jpg', '/en-default-' . ImageType::getFormatedName('small') . '.jpg');
