...
class Cart extends ObjectModel
{
    ...
    public function addCartRule($id_cart_rule)
    {
        ...
        if (!Db::getInstance()->insert('cart_cart_rule', array('id_cart_rule' => (int) $id_cart_rule, 'id_cart' => (int) $this->id))) {
            return false;
        }
    ...
    public function simulateCarriersOutput(Country $default_country = null, $flush = false)
    {
        ...
        foreach (reset($delivery_option_list) as $key => $option) {
            ...
            $carriers[] = array('name' => $name, 'img' => $img, 'delay' => $delay, 'price' => $price, 'price_tax_exc' => $price_tax_exc, 'id_carrier' => Cart::intifier($key), 'is_module' => false);
        }
...
', ''), '', nl2br($text_value));
    ...
    public function setWsCartRows($values)
    {
        if ($this->deleteAssociations()) {
            ...
            foreach ($values as $value) {
                $query .= '(' . (int) $this->id . ', ' . (int) $value['id_product'] . ', ' . (isset($value['id_product_attribute']) ? (int) $value['id_product_attribute'] : 'NULL') . ', ' . (int) $value['quantity'] . ', NOW(), ' . (int) Context::getContext()->shop->id . '),';
            }
    ...
    public static function addExtraCarriers(&$array)
    {
        ...
        foreach (Context::getContext()->cart->getAddressCollection() as $address) {
            ...
            if ($first) {
                $array = array_merge($array, array('HOOK_EXTRACARRIER' => $hook));
                $first = false;
            }
            $array = array_merge($array, array('HOOK_EXTRACARRIER_ADDR' => $hook_extracarrier_addr));
        }
    }
    /**
