...
class RequestSql extends ObjectModel
{
     ...
     */
    public static $definition = array('table' => 'request_sql', 'primary' => 'id_request_sql', 'fields' => array('name' => array('type' => self::TYPE_STRING, 'validate' => 'isString', 'required' => true, 'size' => 200), 'sql' => array('type' => self::TYPE_STRING, 'validate' => 'isString', 'required' => true)));
    ...
    public function cutAttribute($attr, $from)
    {
        if (preg_match('#^((`(\\()?([a-z0-9_])+`(\\))?)|((\\()?([a-z0-9_])+(\\))?))\\.((`(\\()?([a-z0-9_])+`(\\))?)|((\\()?([a-z0-9_])+(\\))?))$#i', $attr)) {
            ...
            if (!($table = $this->returnNameTable($tab[0], $from))) {
                ...
                return array('table' => $table, 'alias' => $tab[0], 'attribut' => $tab[1], 'string' => $attr);
            }
            ...
            if (!($table = $this->returnNameTable(false, $from))) {
                ...
                return array('table' => $table, 'attribut' => $attribut, 'string' => $attr);
            }
        } else {
            return false;
        }
        ...
        for ($i = 0; $i < $nb; $i++) {
            ...
            if ($attribut['expr_type'] == 'colref' || $attribut['expr_type'] == 'reserved') {
                ...
                if ($attribut['expr_type'] == 'operator') {
                    ...
                    if ($attribut['expr_type'] == 'subquery') {
