...
class SupplyOrderDetail extends ObjectModel
{
    ...
    protected function calculatePrices()
    {
        ...
        $this->price_te = Tools::ps_round((double) $this->unit_price_te * (int) $this->quantity_expected, 6);
        ...
        if ($this->discount_rate != null && (is_float($this->discount_rate) || is_numeric($this->discount_rate)) && $this->discount_rate > 0) {
            $this->discount_value_te = Tools::ps_round((double) $this->price_te * ($this->discount_rate / 100), 6);
        }
        ...
        $this->tax_value = Tools::ps_round($this->price_with_discount_te * ((double) $this->tax_rate / 100), 6);
    ...
    {
        if ($discount_rate != null && is_numeric($discount_rate) && (double) $discount_rate > 0) {
            // calculates new price, with global order discount, tax ecluded
            $discount_value = $this->price_with_discount_te - $this->price_with_discount_te * (double) $discount_rate / 100;
            $this->price_with_order_discount_te = Tools::ps_round($discount_value, 6);
            // calculates new tax value, with global order discount
            $this->tax_value_with_order_discount = Tools::ps_round($this->price_with_order_discount_te * ((double) $this->tax_rate / 100), 6);
            parent::update();
        }
    }
    /**
