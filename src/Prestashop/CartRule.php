...
class CartRule extends ObjectModel
{
        ...
        if ($this->minimum_amount) {
            ...
            if ($this->gift_product && $alreadyInCart) {
                ...
                if (Db::getInstance()->getValue($query)) {
                    $ref = false;
                    $product_price = Product::getPriceStatic($this->gift_product, $this->minimum_amount_tax, $this->gift_product_attribute, null, null, false, true, 1, null, $context->cart->id_customer ? $context->cart->id_customer : null, $context->cart->id, (int) $context->cart->{Configuration::get('PS_TAX_ADDRESS_TYPE')} ? (int) $context->cart->{Configuration::get('PS_TAX_ADDRESS_TYPE')} : null, $ref, true, true, $context, true);
    ...
    public function getAssociatedRestrictions($type, $active_only, $i18n)
    {
        ...
        if (!Validate::isLoadedObject($this) or $this->{$type . '_restriction'} == 0) {
            ...
            ' . ($type == 'cart_rule' ? 'AND t.id_cart_rule != ' . (int) $this->id : '') . $shop_list . ' ORDER BY name ASC');
        } else {
            if ($type == 'cart_rule') {
                ...
                WHERE 1 ' . ($active_only ? ' AND t.active = 1' : '') . $shop_list . (in_array($type, array('carrier', 'shop')) ? ' AND t.deleted = 0' : '') . ' ORDER BY name ASC', false);
