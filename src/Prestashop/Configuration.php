    ...
    public static function loadConfiguration()
    {
        ...
        foreach ($results as $row) {
            ...
            if (!isset(self::$_CONF[$lang])) {
                self::$_CONF[$lang] = array('global' => array(), 'group' => array(), 'shop' => array());
            }
            if ($row['id_shop']) {
        ...
        foreach ($values as $lang => $value) {
            ...
            if (Configuration::hasKey($key, $lang, $id_shop_group, $id_shop)) {
                if (!$lang) {
                    ...
                    $result &= Db::getInstance()->update('configuration', array('value' => pSQL($value, $html), 'date_upd' => date('Y-m-d H:i:s')), '`name` = \'' . pSQL($key) . '\'' . Configuration::sqlRestriction($id_shop_group, $id_shop), 1, true);
                } else {
                    // Update multi lang
